...
double qdist (point p1, point p2)
  {double dx = p1.x - p2.x;
   ...
   double dz = p1.z - p2.z;
    ...
    /* check_nil */      {if (p1 == nil_point || p2 == nil_point)
         return DBL_MAX;
     };
...
point rotate (point p, line axis, double angle)
  {vector t;
   ...
   t  = (xx * g_cos (angle)) +
        ( /* vv */ new_vector (axis.p0, axis.p1) * ((xx *  /* vv */ new_vector (axis.p0, axis.p1)) / ( /* vv */ new_vector (axis.p0, axis.p1) *  /* vv */ new_vector (axis.p0, axis.p1))) * (1.0 - g_cos (angle))) -
        (perp (xx,  /* vv */ new_vector (axis.p0, axis.p1) / length ( /* vv */ new_vector (axis.p0, axis.p1))) * g_sin (angle));
    return (axis.p0 + t);
...
point gen (cube c, vector v)
  {return (c.p0 + ( /* f0 */  new_vector (c.p0, c.p1) * v.dx) + ( /* f1 */  new_vector (c.p0, c.p2) * v.dy) + ( /* f2 */  new_vector (c.p0, c.p3) * v.dz));
...
bool inside (triangle t, point p)
  {return (angle ( /* p0p1 */  new_vector (t.p0, t.p1),  /* p0p */   new_vector (t.p0, p)) <= angle ( /* p0p1 */  new_vector (t.p0, t.p1),  /* p0p2 */  new_vector (t.p0, t.p2)) &&
           angle ( /* p1p0 */  new_vector (t.p1, t.p0),  /* p1p */   new_vector (t.p1, p)) <= angle ( /* p1p0 */  new_vector (t.p1, t.p0),  /* p1p2 */  new_vector (t.p1, t.p2)));
...
line expand (line l, double extent)
  {line r;
   ...
   if   (extent < 0)
         /* extent_towards_p0 */      {r = new_line (l.p0 - set_length (new_vector (l.p0,l.p1), -extent), l.p1);
     }
   else  /* extent_towards_p1 */      {r = new_line (l.p0, l.p1 + set_length (new_vector (l.p0, l.p1), extent));
     };
   return r;
...
void print (cube c)
  {if   (c == nil_cube)
        printf ("[NIL]");
   else  /* print_none_nil_cube */      {printf ("[");
      print  (c.p0);
      print  (c.p1);
      print  (c.p2);
      print  (c.p3);
...
point corner (cube c, int corner_no)
  {switch (corner_no)
     {case 0 : return c.p0;
      ...
      case 2 : return c.p2;
      case 3 : return c.p3;
      case 4 : return c.p2 +  /* v1 */ new_vector (c.p0, c.p1);
      case 5 : return c.p3 +  /* v1 */ new_vector (c.p0, c.p1);
      case 6 : return c.p3 +  /* v1 */ new_vector (c.p0, c.p1) +  /* v2 */ new_vector (c.p0, c.p2);
      case 7 : return c.p3 +  /* v2 */ new_vector (c.p0, c.p2);
     };
   return c.p0;
...
point center (cube c)
  {return (c.p0 + ( /* v01 */ new_vector (c.p0, c.p1) * 0.5) + ( /* v02 */ new_vector (c.p0, c.p2) * 0.5) + ( /* v03 */ new_vector (c.p0, c.p3) * 0.5));
...
void edges (cube c, line edge_list [12])
  {point cc [8];
    ...
    /* get_corners */      {for (int i = 0; i < 8; i++)
        cc [i] = corner (c, i);
     };
    /* fill_edge_list */      {edge_list [ 0] = new_line (cc [0], cc [1]);
...
void xplane (cube  c,
  ...
  {point corner_p;
    ...
    /* adjust_plane */      {corner_p = corner (c, corner_no);
      for (int i = 0; i < 4; i++)
         /* check_orient */      {if   ( /* orient_ok */      (corner_p == p0))
           break;
      else  /* rotate */      {point d;
      point p3;
    ...
    /* perhaps_wrong_params */      {if (!  /* orient_ok */      (corner_p == p0)) 
...
void xplane (cube c, int side_no, point &p0, point &p1, point &p2, point &pl)
  {switch (side_no)
     {case 0 : p0 =  /* cp0 */ corner (c, 0); p1 =  /* cp1 */ corner (c, 1); p2 =  /* cp2 */ corner (c, 2); pl =  /* cp3 */ corner (c, 3); break;
      case 1 : p0 =  /* cp3 */ corner (c, 3); p1 =  /* cp5 */ corner (c, 5); p2 =  /* cp0 */ corner (c, 0); pl =  /* cp7 */ corner (c, 7); break;
      case 2 : p0 =  /* cp2 */ corner (c, 2); p1 =  /* cp4 */ corner (c, 4); p2 =  /* cp7 */ corner (c, 7); pl =  /* cp0 */ corner (c, 0); break;
      case 3 : p0 =  /* cp7 */ corner (c, 7); p1 =  /* cp6 */ corner (c, 6); p2 =  /* cp3 */ corner (c, 3); pl =  /* cp2 */ corner (c, 2); break;
      case 4 : p0 =  /* cp5 */ corner (c, 5); p1 =  /* cp6 */ corner (c, 6); p2 =  /* cp1 */ corner (c, 1); pl =  /* cp3 */ corner (c, 3); break;
      case 5 : p0 =  /* cp3 */ corner (c, 3); p1 =  /* cp7 */ corner (c, 7); p2 =  /* cp0 */ corner (c, 0); pl =  /* cp5 */ corner (c, 5); break;
     };
...
bool no_cut (cube c1, cube c2, bool is_dump)
  {line        edge_list [12];
   ...
   edges (c2, edge_list);
   for (int e = 0; e < 12; e++)
     { /* check_c2_edge */      {point p;
      point p1;

       /* perhaps_dump */      {
     };
      if (intersect (c1, edge_list [e], p, p1))
          /* indicate_cut */      { /* perhaps_close_dump */      {
     };
      return false; 
     };
     };
...
bool disjunct (cube c1, cube c2, bool with_trace)
  {return (hulls_disjunct (c1, c2) ||  /* really_disjunct */      (no_cut (c1, c2, with_trace) && no_cut (c2, c1, with_trace)));
...
cube set_size (cube c, double sx, double sy, double sz)
  {vector dx = new_vector (c.p0, c.p1);
   ...
   vector dz = new_vector (c.p0, c.p3);
    ...
    /* modify_lengths */      {if (sx != 0.0) dx = set_length (dx, sx);
...
void hull (cube c, point &min, point &max)
  { /* init */      {min = new_point (DBL_MAX, DBL_MAX, DBL_MAX);
      max = new_point (DBL_MIN, DBL_MIN, DBL_MIN);
     };
   for (int cn = 0; cn < 8; cn++)
      /* check_corner */      {point cor = corner (c, cn);
...
plane bottom (cube c)
  {point cp = center   (c);
   ...
   for (int i = 0; i < 6; i++)
      /* check_plane */      {plane pl = xplane (c, i);
      point p;
...
bool intersect (cube c, line l, point  &ip, point &ip1)
  {ip  = nil_point;
   ip1 = nil_point;
   for (int s = 0; s < 6; s++)
      /* check_plane */      {point pp;

      if (intersect (xplane (c, s), l, pp))
          /* store_pp */      {if   (ip == nil_point)
           ip = pp;
      else ip1 = pp;
     };
     };
  ...
  {line edge_list [12];
   ...
   for (int e = 0; e < 12; e++)
      ...
      if (intersect (c, edge_list [e], p1, p2))
          /* store_pp */      {if (p1 != nil_point)  /* store_p1 */      {for (int i = 0; i < 8; i++)
        if (ip [i] == nil_point)
           {ip [i] = p1;
            break;
           };
     };
      if (p2 != nil_point)  /* store_p2 */      {for (int i = 0; i < 8; i++)
        if (ip [i] == nil_point)
           {ip [i] = p2;
            break;
           };
...
bool intersect (cube c, cube c1, point p [32])
  {bool a;
   bool b;
    ...
    /* perhaps_completly_disjunct */      {if (hulls_disjunct (c, c1))     
         return false;
     };
   a = inters (c, c1, p);
   b = inters (c1, c, p);
...
void check_move (cube   c1,
  ...
  if ( /* points_are_better */      qdist (pp1, pp2) < qdist (p1, p2) &&
     0 <= ppv1.dx && ppv1.dx <= 1 &&
     0 <= ppv1.dy && ppv1.dy <= 1 &&
     0 <= ppv1.dz && ppv1.dz <= 1 &&
     0 <= ppv2.dx && ppv2.dx <= 1 &&
...
void nearest_points (cube c1, cube c2, point &p1, point &p2)
  {bool   step_size_ok = true;
   ...
   vector delta_a;
    ...
    /* init_points */      {delta_a           = new_vector (0.25, 0.25, 0.25);
   ...
   while (step_size_ok)
     { /* delta_climb */      {bool any_improve;

       /* check_moves */      {any_improve = false;
       /* check_last_step */      {check_move (c1,
                  c2,
                  p1,
                  p2,
                  pv [0],
            ...
            for (int y1 = -1; y1 < 2 && ! any_improve; y1++)
              for (int y2 = -1; y2 < 2 && ! any_improve; y2++)
                for (int y3 = -1; y3 < 2 && ! any_improve; y3++)
                  ...
                  any_improve);
       /* perhaps_grab_dir */      {if (any_improve)
          /* grab_dir */      {last_delta [0] = new_vector (x1 * delta_a.dx, 
                                   x2 * delta_a.dy,
                                   x3 * delta_a.dz);
      ...
      last_delta [1] = new_vector (y1 * delta_a.dx, 
       ...
       /* reduce_delta */      {delta_a      = delta_a * 0.1;
      step_size_ok = dist (gen ( /* zp */ new_point (0, 0, 0), v [0], delta_a),  /* zp */ new_point (0, 0, 0)) >= 1 &&
                     dist (gen ( /* zp */ new_point (0, 0, 0), v [1], delta_a),  /* zp */ new_point (0, 0, 0)) >= 1;
     };
     };
     };
...
void check_move (cube   c,
  ...
  if ( /* points_are_better */      qdist (pp, p2) < qdist (p, p2) &&
     0 <= ppv.dx && ppv.dx <= 1 &&
     0 <= ppv.dy && ppv.dy <= 1 &&
     0 <= ppv.dz && ppv.dz <= 1)
       /* grab_points */      {pv          = ppv;
  ...
  {bool   step_size_ok = true;
   ...
   while (step_size_ok)
     { /* delta_climb */      {bool any_improve;
       ...
       /* check_moves */      {any_improve = false;
       /* check_last_step */      {check_move (c, p1, p, pv, v, last_delta, any_improve);
     };
      for (int x1 = -1; x1 < 2 && ! any_improve; x1++)
        for (int x2 = -1; x2 < 2 && ! any_improve; x2++)
          for (int x3 = -1; x3 < 2 && ! any_improve; x3++)
             /* single_step */      {check_move (c,
                  p1,
                  p, 
                  pv,
                  v, 
                  ...
                  any_improve);
       /* perhaps_grab_dir */      {if (any_improve)
          /* grab_dir */      {last_delta = new_vector (x1*delta_a.dx, x2*delta_a.dy, x3*delta_a.dz);
     };
     };                   
     };
     };
      ...
      for (int x1 = -1; x1 < 2 && ! any_improve; x1++)
       ...
       /* reduce_delta */      {delta_a      = delta_a * 0.1;
      step_size_ok = dist (gen ( /* zp */ new_point (0, 0, 0), v, delta_a),  /* zp */ new_point (0, 0, 0)) >= 1;
     };
     };
     };
   return p1;
...
config new_config (double values [])
  {config c;
    ...
    /* enter_values */      {for (int i = 0; i < max_config_size; i++)
        c.v [i] = values [i];
     };
   return c;
...
void print (config c)
  {if   (is_nil (c))
         /* print_nil_config */      {printf ("{NIL}"); 
     }
   else  /* print_config */      {printf ("{");
      for (int i = 0; i < max_config_size; i++)
        {printf ("%f", c.v [i]);
          /* perhaps_seperator */      {if (i < max_config_size - 1)
         printf (", ");
     };
        };
      printf ("}");
...
double qdist (config c1, config c2)
  {double d = 0;
    ...
    /* calc_dist */      {for (int i = 0; i < max_config_size; i++)
         /* add_dist */      {double di;
...
config operator + (config c1, config c2)
  {config r;
    ...
    /* perform_add */      {for (int i = 0; i < max_config_size; i++)
        if   (c1.v [i] == nil_angle || c2.v [i] == nil_angle)
             r.v [i] = nil_angle;
...
config operator - (config c1, config c2)
  {config r;
    ...
    /* perform_sub */      {for (int i = 0; i < max_config_size; i++)
        if   (c1.v [i] == nil_angle || c2.v [i] == nil_angle)
             r.v [i] = nil_angle;
        else r.v [i] = c1.v [i] - c2.v [i];
     };
...
config operator / (config c1, double val)
  {config r;
    ...
    /* perform_div */      {for (int i = 0; i < max_config_size; i++)
        if   (c1.v [i] == nil_angle)
             r.v [i] = nil_angle;
...
config sign (config c)
  {config r;
    ...
    /* perform_sign */      {for (int i = 0; i < max_config_size; i++)
        r.v [i] = d_sign (c.v [i]);
     };
   return r;
...
void print (plane p)
  {if   (p == nil_plane)
        printf ("/NIL/");
   else  /* print_none_nil_plane */      {printf ("/ ");
      print  (p.p0);
      print  (p.p1);
      print  (p.p2);
      printf (" /");
...
bool intersect (plane p, line l, point &ip)
  {double v1v2v3;
   ...
   ip     = nil_point;
   v1v2v3 = det ( /* v1 */   new_vector (p.p0, p.p1),  /* v2 */   new_vector (p.p0, p.p2),  /* v3 */   new_vector (l.p0, l.p1));
   if ( /* is_parallel */      v1v2v3 == 0)
      return false;
   v1p1v3 = det ( /* v1 */   new_vector (p.p0, p.p1), new_vector ( /* pp */   (l.p0 - p.p0).x,  /* pp */   (l.p0 - p.p0).y,  /* pp */   (l.p0 - p.p0).z),  /* v3 */   new_vector (l.p0, l.p1));
   p1v2v3 = det (new_vector ( /* pp */   (l.p0 - p.p0).x,  /* pp */   (l.p0 - p.p0).y,  /* pp */   (l.p0 - p.p0).z),  /* v2 */   new_vector (p.p0, p.p2),  /* v3 */   new_vector (l.p0, l.p1));
   v1v2p1 = det ( /* v1 */   new_vector (p.p0, p.p1),  /* v2 */   new_vector (p.p0, p.p2), new_vector ( /* pp */   (l.p0 - p.p0).x,  /* pp */   (l.p0 - p.p0).y,  /* pp */   (l.p0 - p.p0).z));
   b      = v1p1v3 / v1v2v3;
   a      = p1v2v3 / v1v2v3;
   c      = v1v2p1 / v1v2v3;
   if   ( /* is_inside */      
     ...
     d_abs (c) <= 1)
        {ip = p.p0 + ( /* v2 */   new_vector (p.p0, p.p2) * b) + ( /* v1 */   new_vector (p.p0, p.p1) * a);
         return true;
        }
   else return false;
...
double angle (plane p1, plane p2)
  {return (angle (perp ( /* v11 */ new_vector (p1.p0, p1.p1),  /* v12 */ new_vector (p1.p0, p1.p2)), perp ( /* v21 */ new_vector (p2.p0, p2.p1),  /* v22 */ new_vector (p2.p0, p2.p2))));
...
void add (scene &s, cube c, char c_name [])
  { /* perhaps_scene_overflow */      {if (s.num_elements == max_scene_elements)
         errorstop (46, "OBJECTS", "scene overflow");
     };
    /* add_data */      {s.cubes [s.num_elements] = c;
...
void print (scene s)
  { /* print_name */      {printf ("=%s===================\n", s.name);
    ...
    /* print_elements */      {for (int i = 0; i < s.num_elements; i++)
       {printf ("   ");
        print  (s.cubes [i]);
        printf ("-%s-\n", s.names [i]);
       };
...
cube xcube (scene s, char c_name [])
  {for (int i = 0; i < s.num_elements; i++)
      /* check_element */      {if (strcmp (s.names [i], c_name) == 0)
         return s.cubes [i];
     };
   return nil_cube;

  ...
  {init_cand ();
   for (int x = 0; x < landscape_dx; x++)
     for (int y = 0; y < landscape_dy; y++)
        /* handle_field */      {int ll = landscape [x][y];

       /* set_f */      {if   (unit [x][y] != none)
            /* handle_unit */ 
     {int u = unit [x][y];
...
*/
      ...
      push_neighbors (x, y);
      ...
      else  /* handle_free_f */      {if      (ll == land_water)
              f [x][y] = blocked + 1;  
       ...
       /* set_water */      {is_water [x][y] = (ll == land_water);
     };
     };
  ...
  {for (int x = 0; x < landscape_dx; x++)
     for (int y = 0; y < landscape_dy; y++)
       { /* set_f */      {f [x][y] = 0;
         ...
         /* set_water */      {is_water [x][y] = (landscape [x][y] == land_water);
     };
       };
...
void field::add_forti (int x, int y, int p, int dx, int dy, int p0)
  {if   (dx == 0)
         /* handle_dy */      {for (int i = 0; i < p; i++)
        for (int xs = x - i; xs <= x + i; xs++)
          if (0   <= xs           &&
              xs  <  landscape_dx &&
              0   <=  /* yys */  y + i * dy          &&
               /* yys */  y + i * dy < landscape_dy)
             {f [xs][ /* yys */  y + i * dy] = i_max (f [xs][ /* yys */  y + i * dy], p0 + (p - i));
#ifdef f_trace
              players [0]->show_int (xs,  /* yys */  y + i * dy, f [xs][ /* yys */  y + i * dy]);
#endif
             };
     }
   else  /* handle_dx */      {for (int i = 0; i < p; i++)
        for (int ys = y - i; ys <= y + i; ys++)
          if (0           <=  /* xxs */  x + i * dx          && 
               /* xxs */  x + i * dx         <  landscape_dx &&
              0           <= ys           &&
              ys          < landscape_dy)
             {f [ /* xxs */  x + i * dx][ys] = i_max (f [ /* xxs */  x + i * dx][ys], p0 + (p - i));
#ifdef f_trace
              players [0]->show_int ( /* xxs */  x + i * dx, ys, f [ /* xxs */  x + i * dx][ys]);
#endif
             };
     };
  ...
  {if   (dx == 0)
        ...
        for (int xs = x - i; xs <= x + i; xs++)
          if (0   <= xs           &&
              ...
              0   <=  /* yys */  y + i * dy          &&
               /* yys */  y + i * dy < landscape_dy  &&
              f [xs][ /* yys */  y + i * dy] <= 0)
             {f [xs][ /* yys */  y + i * dy] = i_min (f [xs][ /* yys */  y + i * dy], - (p - i));
#ifdef f_trace
              players [0]->show_int (xs,  /* yys */  y + i * dy, f [xs][ /* yys */  y + i * dy]);
#endif
             };
        ...
        for (int ys = y - i; ys <= y + i; ys++)
          if (0           <=  /* xxs */  x + i * dx          && 
              ...
              ys          < landscape_dy  && 
              f [ /* xxs */  x + i * dx][ys] <= 0)
             {f [ /* xxs */  x + i * dx][ys] = i_min (f [ /* xxs */  x + i * dx][ys], - (p - i));
#ifdef f_trace
              players [0]->show_int ( /* xxs */  x + i * dx, ys, f [ /* xxs */  x + i * dx][ys]);
#endif
             };
  ...
  { /* init */      {init_cand ();
   ...
   for (int x = 0; x < landscape_dx; x++)
     ...
     {int u = unit [x][y];
      ...
      else  /* handle_free_f */      {if   (land_properties [landscape [x][y]].walk_possible)
           f [x][y] = 0;
      else f [x][y] = blocked;
     };
     };
...
bool field::expand (int max_time, int color)
  {bool any_change = false;
   ...
   while (another_cand ())
     { /* handle_cand */      {int x = cand_x [first_cand];
      int y = cand_y [first_cand];

       /* set_pot */      {if      (f [x][y] < blocked)  /* handle_f */      {int p = - INT_MAX;
      int d;

      if   (is_water [x][y])
           d = 3;
      else d = 1;
       /* get_p */      {int xmin = i_max (0,                x - 1); 
      int xmax = i_min (landscape_dx - 1, x + 1); 
      int ymin = i_max (0,                y - 1);
      int ymax = i_min (landscape_dy - 1, y + 1);
      ...
      else if (is_water [x][y])     /* handle_w */      {int p = - INT_MAX;
    ...
    /* get_b_max */      {int b_pot [2];
      ...
      if (u                  != none                     &&
          ...
          is_building (objects->type [u]))
          /* grab_b */      {if      (ff > b_pot [0]    &&
               i_abs (x - b_x [0]) > 2 &&
               i_abs (y - b_y [0]) > 2)
                /* grab_b0 */      {b_pot [1] = b_pot [0];
      b_x   [1] = b_x   [0];
      b_y   [1] = b_y   [0];
      b_pot [0] = ff;
      b_x   [0] = x;
      ...
      if (u                  != none                     &&
          (ff < a_pot [0] || ff < a_pot [1])             &&
           ...
           objects->type  [u] != object_mine)
          /* grab_a */      {if      (ff < a_pot [0]    &&
               i_abs (x - b_x [0]) > 2 &&
               i_abs (y - b_y [0]) > 2)
                /* grab_a0 */      {a_pot [1] = a_pot [0];
      a_x   [1] = a_x   [0];
      a_y   [1] = a_y   [0];
      a_pot [0] = ff;
      a_x   [0] = x;
...
void field::dump (int x, int y)
  { /* open_w */      {if (! w_is_open)
    ...
    /* perform_dump */      { /* show_f */      {int min;
      int max;

       /* get_min_max */      {min =   INT_MAX;
      max = - INT_MAX;
      for (int x = 0; x < landscape_dx; x++)
        for (int y = 0; y < landscape_dy; y++)
          if (f [x][y] < blocked)
...
void field::nbest (int xmin,
  ...
  {int x1;
   ...
   for (x1 = xmin; x1 <= xmax; x1++)
      /* look_up */      {if (i_abs (p0 - f [x1][y1]) < pbest && 
          unit [x1][y1] == none           &&
          ! any_water (x1, y1))
         {xx    = x1;
          yy    = y1;
...
void field::nbest (int &x, int &y)
  {int p0    = f [x][y];
   ...
   for (int j = 0; j < 10; j++)
      /* check_new */      {nbest ( /* xmin */  i_max (0, x - j),  /* ymin */  i_max (0, y - j),  /* xmax */  i_min (landscape_dx - 1, x + j),  /* ymin */  i_max (0, y - j), p0, pbest, xx, yy);
       /* check_over */      {if (pbest == 0)
         {x = xx;
          y = yy;
          return;
         };
     };
      nbest ( /* xmin */  i_max (0, x - j),  /* ymin */  i_max (0, y - j),  /* xmin */  i_max (0, x - j),  /* ymax */  i_min (landscape_dy - 1, y + j), p0, pbest, xx, yy);
       /* check_over */      {if (pbest == 0)
         {x = xx;
          y = yy;
          return;
      ...
      nbest ( /* xmin */  i_max (0, x - j),  /* ymax */  i_min (landscape_dy - 1, y + j),  /* xmax */  i_min (landscape_dx - 1, x + j),  /* ymax */  i_min (landscape_dy - 1, y + j), p0, pbest, xx, yy);
       /* check_over */      {if (pbest == 0)
         {x = xx;
          y = yy;
          return;
      ...
      nbest ( /* xmax */  i_min (landscape_dx - 1, x + j),  /* ymin */  i_max (0, y - j),  /* xmax */  i_min (landscape_dx - 1, x + j),  /* ymax */  i_min (landscape_dy - 1, y + j), p0, pbest, xx, yy);
       /* check_over */      {if (pbest == 0)
         {x = xx;
          y = yy;
          return;
...
void field::nbest (int &x, int &y, field *pf)
  {int p0    = f [x][y];
   ...
   for (int j = 0; j < 10; j++)
      /* check_new */      {nbest ( /* xmin */  i_max (0, x - j),  /* ymin */  i_max (0, y - j),  /* xmax */  i_min (landscape_dx - 1, x + j),  /* ymin */  i_max (0, y - j), p0, pbest, xx, yy, pf);
       /* check_over */      {if (pbest == 0)
         {x = xx;
          y = yy;
          return;
      ...
      nbest ( /* xmin */  i_max (0, x - j),  /* ymin */  i_max (0, y - j),  /* xmin */  i_max (0, x - j),  /* ymax */  i_min (landscape_dy - 1, y + j), p0, pbest, xx, yy, pf);
       /* check_over */      {if (pbest == 0)
         {x = xx;
          y = yy;
          return;
      ...
      nbest ( /* xmin */  i_max (0, x - j),  /* ymax */  i_min (landscape_dy - 1, y + j),  /* xmax */  i_min (landscape_dx - 1, x + j),  /* ymax */  i_min (landscape_dy - 1, y + j), p0, pbest, xx, yy, pf);
       /* check_over */      {if (pbest == 0)
         {x = xx;
          y = yy;
          return;
      ...
      nbest ( /* xmax */  i_min (landscape_dx - 1, x + j),  /* ymin */  i_max (0, y - j),  /* xmax */  i_min (landscape_dx - 1, x + j),  /* ymax */  i_min (landscape_dy - 1, y + j), p0, pbest, xx, yy, pf);
       /* check_over */      {if (pbest == 0)
         {x = xx;
          y = yy;
          return;
   ...
   {bool any_progress = true;
    ...
    while (i_abs (f [x][y]) > 0 && any_progress)
      ...
      if (0 <= xx && xx < landscape_dx && 0 <= yy && yy < landscape_dy)
          /* check_f */      {if (i_abs (f [xx][yy]) < i_abs (pp) && 
...
void field::push_cand (int x, int y)
  {if (0 <= x && x < landscape_dx && 0 <= y && y < landscape_dy)
       /* perform_push */       {cand_x [last_cand] = x;
       cand_y [last_cand] = y;
       in_cand [x][y]     = true;
       last_cand++;
       if (last_cand == 10000)

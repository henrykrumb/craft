...

{
    ...
    /* calc_font */ {
        ...
        {
            strcpy(label_font, b_label_font);
        }
        /*
...
bool button::eval(int &button)
{
    ...
    w->tick();
    /* skip_release_events */ {
        while (w->is_mouse(xe, ye, but) && /* at_button */ (x < xe && xe < x + dx && y < ye && ye < y + dy) && /* is_release */ (but == button1release || but == button2release || but == button3release))
        {
            w->mouse(xe, ye, but);
        }
    };
    ...
    /* perhaps_grab_event */ {
        if (is_mouse && /* at_button */ (x < xe && xe < x + dx && y < ye && ye < y + dy))
        {

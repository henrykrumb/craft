...
button::button (win  *b_w,
   ...
   { /* init_status */      {strcpy (label, b_label);
      w                  = b_w;
      x                  = b_x;
      y                  = b_y;
      dx                 = b_dx;               
     ...
     /* calc_font */      {if   (b_label_font == NULL)
...
*/
     ...
     /* perhaps_calc_size */      if (dx == by_default || dy == by_default)
         /* calc_size */      if   ( /* is_text_menu */      (label [0] != '$' && label [0] != '/'))
           /* calc_text_dim */      {int label_dx;
      int label_dy;
     ...
     else  /* calc_icon_dim */      {char button_name [128];

       /* calc_button_name */      {if   (label [0] == '/')
...
bool button::eval (int &button)
  {int  d;
   ...
   w->tick ();
    /* skip_release_events */      {while (w->is_mouse (xe, ye, but) &&  /* at_button */      (x < xe && xe < x + dx && y < ye && ye < y + dy) &&  /* is_release */      (but == button1release || 
      but == button2release ||
      but == button3release))
        w->mouse (xe, ye, but);
     };
   is_mouse = w->is_mouse (xe, ye, button);
    /* perhaps_grab_event */      {if (is_mouse &&  /* at_button */      (x < xe && xe < x + dx && y < ye && ye < y + dy))
         w->mouse (xe, ye, button);
     };
    /* perhaps_no_repeat */      if (is_mouse &&  /* at_button */      (x < xe && xe < x + dx && y < ye && ye < y + dy) && ! with_repeat)
         /* skip_input */      {w->tick ();
      while (w->mouse (d, d, but))
        {w->tick ();
        };
     };
   return (is_mouse &&  /* at_button */      (x < xe && xe < x + dx && y < ye && ye < y + dy)) ||
          (with_repeat && w->mouse_is_pressed (0) &&  /* at_button */      (x < xe && xe < x + dx && y < ye && ye < y + dy));
...
void button::write (char label [], bool is_pressed)
  { /* draw_border */      {w->function (GXcopy);
      if   (is_pressed)
            /* pressed_border */      { /* clear */      {w->set_color (button_color);
      w->fill      (x, y, dx, dy);
     };
      for (int i = 0; i < dx_border; i++)
         /* draw_pressed_line */      {if   (i <= dx_border / 2)
           w->set_color (black);
      else w->set_color (border_color_light);
      w->line      (   x+i,  y + i,  /* xe */  x + dx - i, y  + i);
      w->line      (   x+i,  y + i, x  + i,  /* ye */  y + dy - i);
      if   (i <= dx_border / 2)
           w->set_color (black);
      else w->set_color (border_color_dark);
      w->line      (x  + i,  /* ye */  y + dy - i,  /* xe */  x + dx - i,  /* ye */  y + dy - i);
      w->line      ( /* xe */  x + dx - i, y  + i,  /* xe */  x + dx - i,  /* ye */  y + dy - i);
     };
     }
      else  /* none_pressed_border */      { /* clear */      {w->set_color (button_color);
      w->fill      (x, y, dx, dy);
     };
      for (int i = 0; i < dx_border; i++)
         /* draw_released_line */      {w->set_color (border_color_light);
      w->line      (   x+i,  y + i,  /* xe */  x + dx - i, y  + i);
      w->line      (   x+i,  y + i, x  + i,  /* ye */  y + dy - i);
   ...
   if   (is_icon)
         /* write_icon */      {char button_name [128];
   ...
   else  /* write_text */      {w->function       (GXcopy);
      w->set_background (button_color);
      w->set_color      (label_color);

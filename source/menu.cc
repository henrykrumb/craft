...
menu::menu (const char w_name [],
  ...
  { /* init_manager */      if (! manager_init)
    ...
    /* load_colors */      {c_border_light = win_default_c ("menu_border_light");
    ...
    /* calc_dim */      if   ( /* is_text_menu */      (w_cmds [0] != '$' && w_cmds [0] != '/'))
           /* calc_text_dim */      { /* scan_buttons */      {int         p = 0;
      char        cmd [128];
      ...
      while (get_cmd (w_cmds, p, cmd))
         /* handle_button */      { /* calc_tdx_tdy */      { /* act_tdx */   tdx [num_buttons] = XTextWidth (font_info, cmd, strlen (cmd));
     ...
     else  /* calc_icon_dim */      {int icon_dx;
      int icon_dy;

       /* calc_button_dir */      {if   (w_cmds [0] == '$')
           strcpy  (button_dir, "");
      else sprintf (button_dir, "%s/", win_default_s ("button_dir"));
     };
       /* scan_buttons */      {int         p = 0;
      char        cmd [128];
      XFontStruct *font_info;
      Display     *display;
...
void menu::open ()
  { /* open_window */     {w = new win (name, "", x, y, dx, dy);
     w->set_font (men_font);
    };
    /* write_cmds */      {for (int i = 0; i < num_buttons; i++)
...
void menu::write (int no, char string [], bool pressed)
  { /* draw_border */      {if   (pressed)
            /* pressed_border */      { /* clear */      {w->set_color (c_background);
      w->fill      ( /* x_button */   0,  /* y_button */    /* yy */   no * button_dy, button_dx, button_dy);
     };
      for (int i = 0; i < border_dx; i++)
         /* draw_pressed_line */      {w->set_color (c_border_dark);
      if (i < border_dx-1)
         w->set_color (black);
      ...
      else  /* none_pressed_border */      { /* clear */      {w->set_color (c_background);
      w->fill      ( /* x_button */   0,  /* y_button */    /* yy */   no * button_dy, button_dx, button_dy);
     };
      for (int i = 0; i < border_dx; i++)
         /* draw_released_line */      {w->set_color (c_border_light);
      w->line      ( /* x_button */   0+i,  /* y_button */    /* yy */   no * button_dy+i,  /* xe_button */   /* x_button */   0 + button_dx - i,  /* y_button */    /* yy */   no * button_dy   + i);
      w->line      ( /* x_button */   0+i,  /* y_button */    /* yy */   no * button_dy+i,  /* x_button */   0  + i,   /* ye_button */   /* y_button */    /* yy */   no * button_dy + button_dy - i);
   ...
   if   (icon_mode)
      ...
      w->write          ( /* xtext */      border_dx + (((button_dx - 2 * border_dx) -  /* act_tdx */    tdx [no]) / 2),  /* ytext */       /* yy */   no * button_dy + 
              border_dy +
               /* act_tdy */    tdy [no] +
              2 +
              (((button_dy - 2 * border_dy) -  /* act_tdy */    tdy [no]) / 2), string);
      w->tick           ();
     };
  ...
  {tick ();
   if   (is_open)
      ...
      if      (button == button1press || button == button3press)
               /* handle_new_press */      {last_mouse_button = button;
      ...
      pressed_button = no_menu_request;
       ...
       /* return_result */      {return pressed_button;
     };
     }
...
bool menu::get_cmd (const char cmds [], int &p, char cmd [])
  {if   ( /* eof */ 
     ...
     cmds [p] != ':' && !  /* eof */ 
     (p >= strlen (cmds)))
        { /* handle_skip_mode */      {if ( /* cmd_char */ 
     cmds [p] == ';')
          /* switch_skip_mode */      {skip_mode = ! skip_mode;
      p++;
     };
     };
          /* store_char */      {if (! skip_mode)
         cmd [c++] =  /* cmd_char */ 
     cmds [p];
     };
         p++;

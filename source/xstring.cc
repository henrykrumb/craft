...
void compress (char s [])
  { /* delete_leading_blanks */      while ( /* first_char */ s [0] == ' ')
        /* delete_first_char */      {for (int i = 0; s [i] != '\0' && s [i] != 0; i++)
        s [i] = s [i+1];
     };
    /* delete_trailing_blanks */      {int i;

       /* skip_to_last_char */      {for (i = 0; s [i] != '\0' && s [i] != 0; i++);
        i--;
     };
      while ( /* last_char */  s [i] == ' ')
...
void inschar (char s[], int pos, char n)
  {if   (pos < strlen (s))
         /* handle_insert */      {for (int i = strlen (s); i >= pos; i--)
        s [i+1] = s [i];
      s [pos] = n;
     }
   else  /* handle_append */      {int l = strlen (s);

  ...
  {FILE *xbm_data;
    ...
    /* get_size */     {char d [128];
     ...
     fscanf (xbm_data, "%127s %127s %d", d, d, &dy);
      /* skip_line */      {while (fgetc (xbm_data) != '\n')
        {
        };
     };
      /* skip_line */      {while (fgetc (xbm_data) != '\n')
    ...
    /* alloc_data */      {data = new int [dx * dy];
     };
    /* read_pixels */      {int b;

      for (int y = 0; y < dy; y++)
        for (int x = 0; x < dx; x++)
           /* read_pixel */      { /* perhaps_get_b */      {char c;

      if (x % 8 == 0)
          /* get_new_b */      {char bb [128];
...
void xbm::save (char name [])
  {FILE *f;
    ...
    /* write_size */      {fprintf (f, "#define x_width %d\n", dx);
    ...
    /* write_data */      {int c = 0;
      ...
      for (y = 0; y < dy; y++)
        for (x = 0; x < dx; x++)
          { /* write_byte */      { /* perhaps_flush */      {if ( /* on_byte */      ((x % 8) == 0) && ! (x == 0))
          /* flush_byte */      {if (! (x == 8 && y == 0))
         fprintf (f, ", ");
      if (c % 12 == 0)
         fprintf (f, "\n");      c++;
      ...
      if (data [ind (x, y)])
          b = b_set (b, x % 8);
            ...
            /* perhaps_flush_last_byte */      {if (!  /* on_byte */      ((x % 8) == 0) && x == dx-1)
          /* flush_byte */      {if (! (x == 8 && y == 0))
         fprintf (f, ", ");
      if (c % 12 == 0)
         fprintf (f, "\n");      c++;

...
ppm::ppm (int pdx, int pdy, int r, int g, int b)
  { /* set_status */      {dx       = pdx;
    ...
    /* alloc_data */      {r_data = new int [dx * dy];
      g_data = new int [dx * dy];
      b_data = new int [dx * dy];
     };
    /* init_data */      {for (int i = 0; i < dx * dy; i++) 
        {r_data [i] = r;
         g_data [i] = g;
         b_data [i] = b;
        };
...
ppm::ppm (ppm *p)
  { /* copy_status */      {dx       = p->dx;
    ...
    /* copy_data */      {for (int i = 0; i < dx * dy; i++) 
        {r_data [i] = p->r_data [i];
         g_data [i] = p->g_data [i];
         b_data [i] = p->b_data [i];
        };
...
ppm::ppm (char name [], bool with_cache)
  {FILE *data;
   bool is_bin;
    ...
    /* init_cache */      {is_cache     = with_cache;
      cache_loaded = false;
     };
    /* open_data */      {char f_name [128];

      strcpy (f_name, name);
      f_open (data, f_name, "r");
     };
    /* get_size */     {char type   [128]; 
     int  color;
     char buffer [1024];

      /* get_a_line */      {f_getline (data, buffer, 1024);
      while (buffer [0] == '#')
        {f_getline (data, buffer, 1024);
        };
     };
     sscanf (buffer, "%127s", type);
      /* check_type */      {if      (strcmp (type, "P3") == 0)
              is_bin = false;
    ...
    /* read_pixels */      {if   (is_bin)
            /* read_bin_pixels */      {for (int y = 0; y < dy; y++)
        for (int x = 0; x < dx; x++)
           /* read_bin_pixel */      {conv_data c;
      ...
      else  /* read_plain_pixels */      {for (int y = 0; y < dy; y++)
        for (int x = 0; x < dx; x++)
           /* read_pixel */      {fscanf (data, "%d %d %d",
              &r_data [ind (x, y)],
              &g_data [ind (x, y)],
              &b_data [ind (x, y)]);
     }; 
...
void ppm::save (char name [])
  {FILE *f;
    ...
    /* open_f */      {f_open (f, name, "w");
     };
    /* write_header */      {fprintf (f, "P6\n");
     };
    /* write_size */      {fprintf (f, "%d %d\n", dx, dy);
      fprintf (f, "255\n");
     };
    /* write_data */      {for (int y = 0; y < dy; y++)
         /* write_line */      {conv_data c;
...
void ppm::show (win *w, int wx, int wy)
  {if   (is_cache)
         /* show_with_cache */      { /* perhaps_load_cache */      {if (! cache_loaded) 
          /* perform_load_cache */      {for (int x = 0; x < dx; x++)
        for (int y = 0; y < dy; y++)
           /* load_pixel */      {int i = ind (x, y);
      ...
      for (int x = 0; x < dx; x++)
         for (int y = 0; y < dy; y++)
            /* show_cache_pixel */      {w->set_color (cache [ind (x, y)]);
      w->pixel     (wx + x, wy + y);
     };
     }
   else  /* show_without_cache */      {for (int x = 0; x < dx; x++)
        for (int y = 0; y < dy; y++)
           /* show_pixel */      {int i = ind (x, y);
...
void ppm::show (win *w,
  ...
  {int yy = wy;
   ...
   w->function (GXcopy);
    /* perhaps_load_cache */      {if (! cache_loaded) 
          /* perform_load_cache */      {for (int x = 0; x < dx; x++)
        for (int y = 0; y < dy; y++)
           /* load_pixel */      {int i = ind (x, y);
...
int ppm::ind (int x, int y)
  {if   (0 <= x && x < dx && 0 <= y && y <= dy)
        return y * dx + x;
   else  /* handle_error */      {printf    ("(x,y) out of range %d %d\n", x, y);
...
void copy (ppm *dest, ppm *src)
  { /* check_size */      {if (dest->dx != src->dx || dest->dy != src->dy)
         errorstop (1, "ppm", "incompatible types at copy\n");
     };
    /* perform_copy */      {for (int i = 0; i < src->dx * src->dy; i++) 

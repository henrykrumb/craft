...
void lru::access (int id)
  {if (id != fresh)
       /* perform_access */      { /* remove_from_queue */      {if   (cand == id) 
           cand = pred [id];
      else pred [suc  [id]] = pred [id];
      if  (fresh == id)
           fresh = suc [id];

...
double d_get (FILE *f, bool &is_eof) 
  {char buf [40];
   ...
   int  c;
    ...
    /* skip_delimiter */      while ( /* read_char */      (c = (buf [ptr++] = getc (f))) == ' ' || c == '\n')
       ptr = 0;
   while ( /* read_char */      (c = (buf [ptr++] = getc (f))) != ' ' && c != EOF && c != '\n');
   buf [ptr] = '0';
   is_eof = (c == EOF);
   return atof (buf);
...
void f_open_with_extend (FILE *&f, char name [], char mode [], int max_no)
  {int  version;
   char v_name [128];
    ...
    /* get_version */      {for (version = 0; version < max_no; version++)
         /* check_v_name */      {sprintf (v_name, "%s.%d", name, version);
      if (! f_exists (v_name))
         break;
     };
      if (version >= max_no)
          /* tell_oh_boy_too_much */      {char msg [256];
...
bool sel_get_name (char name      [],
  ...
  {char          pat [256];
   file_selector *fsel;
    ...
    /* open_fsel */      {strcpy (pat, f_pattern);
      fsel = new file_selector (name, by_fix, by_fix,
                                f_name, pat,  /* is_read_access */      (strcmp (mode, "r") == 0));
     };
    /* perform_select */      {while (! fsel->eval (f_name))
        {
        };
     };
   delete (fsel);
    /* handle_result */      { /* perhaps_complete_file_name */      {char correct_tail [128];

      strcpy (correct_tail, substring (f_pattern, 1));
      if (( /* is_write_access */      (strcmp (mode, "w") == 0) ||  /* is_read_write_access */      ((strcmp (mode, "rw") == 0) || (strcmp (mode, "wr") == 0))) &&  /* wrong_tail */      (strcmp ( /* name_tail */ 
     (substring (f_name, strlen (f_name) - strlen (correct_tail))), correct_tail) != 0))
          /* complete_file_name */      {strcat (f_name, correct_tail);
     };
     };
       /* perhaps_check_overwrite */      {if ( /* is_write_access */      (strcmp (mode, "w") == 0))
          /* check_overwrite */      {if (f_exists (f_name) && ! yes ("overwrite existing file"))
         return false;
     };
     };
      if   (f_name [0] == 0)
...
char *f_postfix (char full_path [])
  {       int  i;
   ...
   r [0] = 0;
    /* skip_to_point */      {for (i = strlen (full_path); i >= 0; i--)
        if (full_path [i] == '.')
           break;
     };
    /* grab_tail */      {if (i >= 0)
...
bool bscanf (FILE *f, int num_bytes, int &i)
  {conv_data c;
   bool      was_eof;
    ...
    /* read_data */      {if   (num_bytes == 1) 
            /* get_byte */      {c.b     = 0;
      was_eof = ((c.a.b3 = fgetc (f)) == EOF);
      i       = c.b;
     }
      else  /* get_int */      {was_eof = ((c.a.b3 = fgetc (f)) == EOF);

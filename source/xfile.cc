...

{
    ...
    delete (fsel);
    /* handle_result */      { /* perhaps_complete_file_name */      {
        ...
        /* perhaps_check_overwrite */      {
            if ( /* is_write_access */      (strcmp (mode, "w") == 0))
                /* check_overwrite */      {
...
bool bscanf (FILE *f, int num_bytes, int &i) {
    ...
    /* read_data */      {
        if   (num_bytes == 1)
            ...
            was_eof = ((c.a.b3 = fgetc (f)) == EOF);
            if (! was_eof) {
                was_eof = ((c.a.b2 = fgetc (f)) == EOF);
            }
            if (! was_eof) {
                was_eof = ((c.a.b1 = fgetc (f)) == EOF);
            }
            if (! was_eof) {
                was_eof = ((c.a.b0 = fgetc (f)) == EOF);
            }
            i       = c.b;
        };
    };

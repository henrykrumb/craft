...
void load_units (char name [])
  {FILE *f;
   int  num_units;
    ...
    /* open_f */      {f_open (f, complete (name, ".units"), "r");
     };
    /* get_size */      {fscanf (f, "%d", &num_units);
     };
    /* read_units */      {for (int i = 0; i < num_units; i++)
         /* read_unit */      {int type;
      int x;
      int y;
      int color;
...
void load_land (char name [])
  {FILE *f;
   FILE *f1;
    ...
    /* open_f */      {f_open (f,  complete (name, ".land"),   "r");
    ...
    /* read_landscape */      {for (int y = 0; y < landscape_dy; y++)
        for (int x = 0; x < landscape_dx; x++)
   ...
   fclose (f);
   fclose (f1);
    /* adjust_pics */      {for (int x = 0; x < landscape_dx; x++)
        for (int y = 0; y < landscape_dy; y++)
           /* check_field */      {landpic     [x][y]    = landscape [x][y];
      landoverlay [x][y][0] = none;
      landoverlay [x][y][1] = none;
      if (land_properties [landscape [x][y]].with_hl)  
          /* apply_hl */      {landpic [x][y] += land_profile (x, y, 1);
     };
     };
     };
...
void heap (int xx, int yy, int h, bool up)
  {if ( /* inside */      (0 <= xx && xx < landscape_dx && 0 <= yy && yy < landscape_dy))
      ...
      if (h >= -1)
          /* set_ilid */      {int best_dist = INT_MAX;
      int id;

      for (x =  /* xmin */ i_max (0, xx - 3); x <  /* xmax */ i_min (landscape_dx - 1, xx + 4); x++)
        for (y =  /* ymin */ i_max (0, yy - 3); y <  /* ymax */ i_min (landscape_dy - 1, yy + 4); y++)
          if (ilid      [x][y] != -1 &&
              landhight [x][y] >  0 &&
               /* act_dist */      ((xx - x) * (xx - x) + (yy - y) * (yy - y))         <  best_dist)
             {id        = ilid [x][y];
              best_dist =  /* act_dist */      ((xx - x) * (xx - x) + (yy - y) * (yy - y));
             };
      ...
      for (x = i_max (0, xx-1); x < i_min (landscape_dx, xx+2); x++)
        for (y = i_max (0, yy-1); y < i_min (landscape_dy, yy+2); y++)
          if (x != xx || y != yy)
              /* check_field */      {if      (h-landhight [x][y] >  1 && up)
              heap (x,y,h-1, up);
...
void rnd_land ()
  { /* rnd_size */      {landscape_dx = i_random (min_rland_dx, max_rland_dx);
      landscape_dy = i_random (min_rland_dy, max_rland_dy);
      if   (landscape_dx > 120 || landscape_dy > 120)
           landscale = 1;
      else landscale = 2;
     };
    /* init_land */      {for (int x = 0; x < landscape_dx; x++) 
        for (int y = 0; y < landscape_dy; y++)
          {landhight   [x][y]    = 1;
    ...
    /* rnd_hight */      {int minh;
      ...
      for (int hi = i_random (minh, maxh); hi > 0; hi--)
        { /* gen_hill */      {int h = i_random (1, 5);
      int x;
      int y;
      int dxx = i_random (6-h, 12 - h);
      int dyy = i_random (6-h, 12 - h);

       /* rnd_xy */      {x = i_random (1, landscape_dx - 2);
    ...
    /* rnd_rivers */      {int num_r;
      ...
      for (int r = i_random (0, num_r); r > 0; r--)
         /* gen_one_river */      {if   (i_random (1, 100) > 50)
            /* lr_river */      {int x  = 0;
      ...
      int b  = i_random (2, bb);
       ...
       /* river */      {while (x != landscape_dx && y != landscape_dy )
        { /* rstep */      { /* rset */      {if    (dx == 0)
    ...
    /* rnd_mines */      {int nn;
      ...
      int m;
       ...
       /* rnd_mxy */      {x = i_random (landscape_dx / nn * xm-3, landscape_dx / nn * xm+3);
    ...
    /* rnd_trade_points */      {int num_t_wood = 2;
      int num_t_gold = 2;
      int xt;
      int yt;

      xt = landscape_dx / 2; yt = 0;                 /* set_trade_point */      {int x;
      int y;
      int tp;

       /* get_tp */      {if      (num_t_wood > 0 && num_t_gold > 0)
      ...
      xt = landscape_dx / 2; yt = landscape_dy - 1;  /* set_trade_point */      {int x;
      int y;
      int tp;
      ...
      xt = 0;                yt = landscape_dy / 2;  /* set_trade_point */      {int x;
      int y;
      int tp;
      ...
      xt = landscape_dx - 1; yt = landscape_dy / 2;  /* set_trade_point */      {int x;
      int y;
      int tp;
    ...
    /* rnd_wood */      {int f = landscape_dx * landscape_dy;
...
void island (int i, int x, int y, int &size, int length)
  {int cx [10000];
   ...
   while ( /* another_cand */      (c1 > c0))
     { /* heap_a_bit */      {if  ( /* cand_inside */      (0 <= cx [c0] && cx [c0] < landscape_dx &&
      0 <= cy [c0] && cy [c0] < landscape_dy))
           /* handle_cand */      {if (landscape [cx [c0]][cy [c0]] == land_sea)
      ...
      s++;
      if (s < size && c1 < 10000)
          /* push_new_cands */      {for (int xx = cx [c0] - 1; xx <= cx [c0] + 1; xx++) 
  ...
  {for (int x = 1; x < landscape_dx-1; x++)
     for (int y = 1; y < landscape_dy-1; y++)
       if (ilid [x][y] != -1 && ilid [x][y] != 57) 
           /* check_field */      {bool border = false;

       /* check_border */      {for (int xx = x - 1; xx < x + 2 && ! border; xx++)
     ...
     { /* session */      {any_hit = false;
      ...
      for (int xx = x - 1; xx <= x+1; xx++)
        for (int yy = y - 1; yy <= y+1; yy++)
          if (xx == x || yy == y)
           /* check_f */      {if (xx < 0 || xx >= landscape_dx ||
...
void island_rnd (int i, int &x, int &y)
  {int k;
    ...
    /* rnd */      {k = i_random (0, ss [i]-1);
      x = xx_il [i][k];
      y = yy_il [i][k];
     };
   while (!  /* ok */     (landhight [x][y] > 0 && unit [x][y] == none))
     { /* rnd */      {k = i_random (0, ss [i]-1);
      x = xx_il [i][k];
      y = yy_il [i][k];
     };
...
void rnd_water_land ()
  {int l;
    ...
    /* rnd_size */      {landscape_dx = i_random ( /* mindx */ (int) ((double) min_rland_dx * 1.0),  /* maxdx */ (int) ((double) max_rland_dx * 1.0));
      landscape_dy = i_random ( /* mindy */ (int) ((double) min_rland_dy * 1.5),  /* maxdy */ (int) ((double) max_rland_dy * 1.5));
      if   (landscape_dx > 120 || landscape_dy > 120)
           landscale = 1;
      else landscale = 2;
     };
    ...
    /* set_islands */      {num_il = i_random (i_max (num_players, 3), 6);
      for (int i = 0; i < num_il; i++)
         /* gen_island */      {int ll = i_random (1, 5);

       /* get_ix_iy */      {x_il [i] = i_random (0, landscape_dx - 1);
    ...
    /* rnd_hight */      {for (int i = 0; i < num_il; i++)
        for (int hi = i_random (4, 10); hi > 0; hi--)
          { /* gen_hill */      {int h = i_random (1, 3);
      int x;
      int y;
      int dxx = i_random (6-h, 12 - h);
      int dyy = i_random (6-h, 12 - h);

       /* rnd_mxy */      {island_rnd (i, x, y);
     };
      for (int xx=i_max (0,x-dxx); xx<i_min (landscape_dx-1,x+dxx); xx++)
        for (int yy=i_max(0,y-dyy);yy<i_min (landscape_dy-1,y+dyy); yy++)
          {heap (xx, yy, h, true);
       ...
       /* readjust_land */      {for (int x = 0; x < landscape_dx; x++)
        for (int y = 0; y < landscape_dy; y++)
          {if (landhight [x][y] > 0 &&
    ...
    /* expand_hight */      {for (int y = 2; y < landscape_dy - 3; y++)
        for (int x = 2; x < landscape_dx - 3; x++)
          if (landhight [x][y] > 1)
              /* check_h_field */      {bool any_water;
 
       /* check_any_water */      {any_water = false;
   ...
   prop_il_id ();
   seperate_islands ();
    /* rnd_mines */      {for (int i = 0; i < num_il; i++)
        for (int z = 0; z < 3; z++)
          { /* set_mine */      {int x;
      int y;
      int m;
    ...
    /* rnd_trade_points */      {int num_t_wood = 2;
      ...
      xt = landscape_dx / 2; yt = 0;                dx=0;dy=1;  /* set_trade_point */      {int x;
      int y;
      int tp;
      int h;
      int s;
      ...
      xt = landscape_dx / 2; yt = landscape_dy - 1; dx=0;dy=-1; /* set_trade_point */      {int x;
      int y;
      int tp;
      int h;
      int s;
      ...
      xt = 0;                yt = landscape_dy / 2; dx=1;dy=0; /* set_trade_point */      {int x;
      int y;
      int tp;
      int h;
      int s;
      ...
      xt = landscape_dx - 1; yt = landscape_dy / 2; dx=-1;dy=0; /* set_trade_point */      {int x;
      int y;
      int tp;
      int h;
      int s;
    ...
    /* rnd_wood */      {int j;
      ...
      int y;
       ...
       /* rnd_wxy */      {int k = i_random (0, ss [i]-1);
          ...
          if (landscape [xx][yy] == land_grass)
           ...
           {landpic [xx][yy] = land_wood;
            ...
            landscape [xx][yy] = land_wood - 1;
               ...
               /* reheap */       {for (int xa = i_max (0, xx-3); xa <i_min (landscape_dx-1, xx+4); xa++)
   ...
   seperate_islands ();
    ...
    /* deep_rivers */      {for (int x = 0; x < landscape_dx; x++)
         /* fill_up_col */      {int last_y;
      int y;

      last_y = 0;
      for (y = 0; y < landscape_dy && landhight [x][y] < 1; y++);
       /* perhaps_fill_up_y */      {if (y - last_y > 1 && y - last_y < 4) 
          /* fill_y */      {for (int yy = last_y; yy < y; yy++)
        {heap (x, yy, 1, true);
        };
     };
     };
      ...
      for (int y = 0; y < landscape_dy; y++)
         /* fill_up_row */      {int last_x;
      int x;

      last_x = 0;
      for (x = 0; x < landscape_dx && landhight [x][y] < 1; x++);
       /* perhaps_fill_up_x */      {if (x - last_x > 1 && x - last_x < 4) 
          /* fill_x */      {for (int xx = last_x; xx < x; xx++)
        {heap (xx, y, 1, true);
        };
     };
     };
...
void save_land (char name [])
  {FILE *f;
   ...
   FILE *f2;
    ...
    /* open_f */      {f_open (f,  complete (name, ".land"),   "w");
    ...
    /* write_landscape */      {for (int y = 0; y < landscape_dy; y++)
        {for (int x = 0; x < landscape_dx; x++)
...
void load_land_props ()
  { /* set_defaults */      {for (int i = 0; i < max_land_types; i++)
    ...
    /* load_properties */      {FILE *infos;
      int  lno;

       /* open_infos */      {f_open (infos, info_name, "r");
     };
       /* skip_first_line */      {while (fgetc (infos) != '\n')
        {
        };
     };
      while ( /* another_property */      (fscanf (infos, "%d", &lno) != EOF))
        { /* load_info */      {char d [128];
...
void land_pop (int id, int x, int y)
  {if ( /* inside */ 
     (0 <= x && x < landscape_dx && 0 <= y && y < landscape_dy))
       /* perform_pop */      {for (int j = 0; j < 8; j++)
        if (upic [x][y][j] == id)
            /* remove */      {upic [x][y][j] = none;
     };
     };
...
int land_profile (int x, int y, int min_d)
  {int h =  /* l */ landhight [x][y];
   int p = 0;

   if   (h <= 0)
        return 0;
   else  /* handle_up_and_down */      {if (0   <= y-1          && h -  /* l */ landhight [x]  [y-1] >= min_d) p += 1;
      if (0   <= x-1          && h -  /* l */ landhight [x-1][y]   >= min_d) p += 10;
      if (y+1 <  landscape_dy && h -  /* l */ landhight [x]  [y+1] >= min_d) p += 100;
      if (x+1 <  landscape_dx && h -  /* l */ landhight [x+1][y]   >= min_d) p += 1000;

...
void heap(int xx, int yy, int h, bool up)
{
    ...
    /* perform_heap */ {
        ...
        if (((up && landhight[xx][yy] < h) || (!up && landhight[xx][yy] > h)) && (h > 0 || unit[xx][yy] == none))
        {
            landhight[xx][yy] = h;
        };
        if (h >= -1)
...
void rnd_land()
{
    ...
    /* rnd_trade_points */ {
        ...
        xt = landscape_dx / 2;
        yt = landscape_dy - 1; /* set_trade_point */
        {
            int x;
            int y;
            int tp;
        ...
        xt = 0;
        yt = landscape_dy / 2; /* set_trade_point */
        {
            int x;
            int y;
            int tp;
...
void island(int i, int x, int y, int &size, int length)
{
    ...
    {
        /* heap_a_bit */ {
            if (/* cand_inside */ (0 <= cx[c0] && cx[c0] < landscape_dx &&
...
void rnd_water_land()
{
    ...
    /* rnd_size */ {
        landscape_dx = i_random(/* mindx */ (int)((double)min_rland_dx * 1.0), /* maxdx */ (int)((double)max_rland_dx * 1.0));
                ...
                /* check_h_field */ {
                    ...
                    /* check_any_water */ {
                        ...
                        for (int xx = i_max(0, x - 3); !any_water &&
                                                       xx < i_min(landscape_dx, x + 4);
                             xx++)
                            for (int yy = i_max(0, y - 3); !any_water &&
                                                           yy < i_min(landscape_dy, y + 4);
                                 yy++)
                            {
...
int land_profile(int x, int y, int min_d)
{
    ...
    { /* handle_up_and_down */
        if (0 <= y - 1 && h - /* l */ landhight[x][y - 1] >= min_d)
        {
            p += 1;
        }
        if (0 <= x - 1 && h - /* l */ landhight[x - 1][y] >= min_d)
        {
            p += 10;
        }
        if (y + 1 < landscape_dy && h - /* l */ landhight[x][y + 1] >= min_d)
        {
            p += 100;
        }
        if (x + 1 < landscape_dx && h - /* l */ landhight[x + 1][y] >= min_d)
        {
            p += 1000;
        }

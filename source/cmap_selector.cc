    ...
    /* handle_list_event */      {
        if (on ())
            ...
            if ( /* any_press_event */      (button == button1press ||
                ...
                break;
                case button2press : { /* handle_unmark */
                    set_mark ( /* event_case */      (ym - y) / case_dy + sc_pos, false);
                    w->tick  ();
                    any_select = true;
                };
                break;
                case button3press : { /* handle_quit */
                    any_select = true;
                    is_quit    = true;
                };
                break;
    ...
    /* read_file_list */      {
        ...
        while ( /* another_name */      (fscanf (l_file, "%1023s", f_line) != EOF &&
            ...
            if (p_match (f_line, file_pattern))
                /* store_name */      {
                sprintf ( /* act_f_name */      file_list [num_files], "%s/%s", file_name, f_tail (f_line));

...
player::player (int    pno,
  ...
  { /* store_params */      {strcpy (name, l_name);
      strcpy (host, l_host);
      color = l_color;
      p_no  = pno;
     };
    /* init_status */      {num_cmds              = 0;
      running_cmd           = none;
      money                 = 1000;
      wood                  = 1000;
      food                  = 250;
   ...
   if   (! is_robot)
         /* open_windows */      { /* init_sun_cnt */      {for (int x = 0; x < max_land_dx; x++)
        for (int y = 0; y < max_land_dy; y++)
          sun_cnt [x][y] = 0;
     };
       /* open_w_craft */      {w_craft = new win  ("craft", host, 0, 0, w_craft_dx, w_craft_dy);
      w_craft->set_color (gray80);
      w_craft->fill      (0, 0, w_craft_dx, w_craft_dy);      
      w_craft->tick      ();
     };
       /* open_w_land */      {char f_name [256];
...
*/
      ...
      for (int i = 0; i < num_players; i++)
       ...
       /* open_w_unit */      {w_unit = new win (w_craft, "craft_units", host,
      ...
      act = 0;  
       ...
       /* open_w_msg */      {w_msg = new win     (w_craft, "msg", host, 0,
                           w_craft_dy - w_msg_dy, w_msg_dx, w_msg_dy);
      g_msg = new craft_getline ("msg", w_msg, message, 1, 1,
                           w_msg_dx-2, w_msg_dy-2, false, false, true);
     };
       /* load_pictures */      {load_pics (); 
     };
       /* open_quit */      {b_quit  = new button (w_status, "quit", 90, 640);
     };
       /* perhaps_open_debug */      {is_debug = false;
      debug_on = false;
   ...
   else  /* init_rob */      {rob        = new robot (l_color, pno);
      bonus      = robo_power;
...
void player::deactivate ()
  {if (! is_robot)
       /* perform_delete */      { /* delete_units */      {for (int i = 0; i < max_objects; i++)
        if (! objects->is_free [i] && objects->color [i] == color)
            /* delete_object */      {switch (objects->type [i])
        {case object_home          : 
       ...
       /* delete_pics */      {for (int i = 0; i < max_pics; i++)
        if (pic_used [i])
           {delete (pics      [i]);
            delete (ship_pics [i]);
           };
     };
       /* delete_cmds */      {for (int i = 0; i < num_cmds; i++)
        if (cmd_active [i])
           delete (cmds [i]);
      if (is_debug)
         delete (b_debug);
...
void player::load_pics ()
  {FILE *pic_catalog;
   int  pno;
    ...
    /* open_pic_catalog */      {f_open (pic_catalog, pic_cata_name, "r");
     };
    /* init_color_trans */      {for (int m = 0; m < num_players; m++)
        { /* set_trans */      {ct [m] = new color_trans ();
      if (m == 1)
         {ct [m]->add (21, 1);
          ct [m]->add (22, 2);
          ct [m]->add (23, 2);
    ...
    /* init */      {for (int i = 0; i < max_pics; i++)
        pic_used [i] = false;
     };
   while ( /* another_pic */      fscanf (pic_catalog, "%d", &pno) != EOF)
      /* load_pic */      {char pic_name [128];

      sprintf (pic_name, "%s.%d.cmap", pic_names, pno);
      if   (pno < 1000)
            /* add_objects */      {pics      [pno] = new cmap (w_land,   pic_name, false, true);
      ship_pics [pno] = new cmap (w_status, pic_name, false, true);
      pic_used  [pno] = true;
     }
      else  /* add_man */      {for (int m = 0; m < num_players; m++)
        { /* set_man */      {char pic_name [128];
...
void player::show ()
  {if (! is_robot)
       /* perform_show */      {for (int x = x0; x <  /* max_x */  i_min (landscape_dx, (x0 + (wdx / pic_dx))); x++)
...
void player::show_int (int lx, int ly, int i)
  {if (! is_robot)
       /* perform_show */      {int  wx = (lx - x0) * pic_dx;
      int  wy = (ly - y0) * pic_dy;
      char s [128];
...
void player::show (int lx0, int ly0, int lx, int ly, bool extra_mark)
  {if (! is_robot &&  /* inside */ 
     (0 <= lx && lx < landscape_dx && 0 <= ly && ly < landscape_dy))
       /* perform_show */      {if   ( /* sunny */      (! is_suny || sun_cnt [lx][ly] > 0))
            /* show_field */      {int wx = (lx - x0) * pic_dx;
      int wy = (ly - y0) * pic_dy;
      int uu = unit [lx][ly];

       /* show_land */      {int l = landpic [lx][ly];

      if   (0 <= l && l < max_pics && pic_used [l])
            /* show_land_pic */      {int ll;
      ...
      for (int i = 0; i < 2; i++)
       ...
       /* show_upics */      {for (int p = 0; p < 8; p++)
         /* perhaps_show_upics */      {int u = upic [lx][ly][p];

      if (u != none && ly - y0 < 24)
          /* show_upic */      {if   (! (0 <=  /* u_pic */  objects->pic [u] &&  /* u_pic */  objects->pic [u] < max_pics && pic_used [ /* u_pic */  objects->pic [u]]))
      ...
      if   ( /* is_msg */      (objects->type [u] == object_msg && ! objects->is_free [u]))
            /* show_msg */      {int wwx = (objects->x [u] - x0) * pic_dx;
      ...
      if   (strlen ( /* msg */   objects->name [u]) > 10)
            /* two_msgs */      {w_land->write (wwx + 10, wwy + pic_dy - 19, substring ( /* msg */   objects->name [u], 0, 10));
      w_land->write (wwx + 10, wwy + pic_dy -  5, substring ( /* msg */   objects->name [u], 10, strlen ( /* msg */   objects->name [u])));
     }
      else  /* single_msg */      {w_land->write (wwx + 10, wwy + pic_dy - 10,  /* msg */   objects->name [u]);
     };
     }
      else  /* show_normal_upic */      {if   (0 <=  /* u_pic */  objects->pic [u] &&  /* u_pic */  objects->pic [u] < max_pics && pic_used [ /* u_pic */  objects->pic [u]])
            /* show_unit */      {int uu = unit [lx][ly];
     ...
     (p                   <  1            &&  
      ...
      objects->wood  [uu] == 1))  /* show_scout_pic */      {pics [land_bush]->show ( /* u_wx */   (objects->wx  [u] - x0 * pic_dx),  /* u_wy */   (objects->wy  [u] - y0 * pic_dy), wx, wy);  
     }
      else if ( /* is_ship */      (p                  <  1    &&
      uu                 != none &&
      objects->type [uu] == object_ship1))          /* show_ship_pic */      {int x_source = wx -  /* u_wx */   (objects->wx  [u] - x0 * pic_dx);
      ...
      else                       /* show_normal_pic */      {pics [ /* u_pic */  objects->pic [u]]->show ( /* u_wx */   (objects->wx  [u] - x0 * pic_dx),  /* u_wy */   (objects->wy  [u] - y0 * pic_dy), wx, wy);
/*
...
*/
     ...
     {if (objects->is_marked [u])
          /* handle_m */      {if (objects->color [u] == color)
         {if   (u == marked [act][0])
               show_main_mark (u,  /* u_wx */   (objects->wx  [u] - x0 * pic_dx),  /* u_wy */   (objects->wy  [u] - y0 * pic_dy));
          else show_mark      (u,  /* u_wx */   (objects->wx  [u] - x0 * pic_dx),  /* u_wy */   (objects->wy  [u] - y0 * pic_dy));
         };
     };
     };
     };
       ...
       /* perhaps_extra_mark */      {if (extra_mark && running_cmd != none)  
          /* show_extra_mark */      {if   (objects->can_built (marked [act][0],
      ...
      else  /* show_black */      {int wx = (lx - x0) * pic_dx;
      int wy = (ly - y0) * pic_dy;
...
void player::show_main_mark (int u, int wx, int wy)
  {if (! is_robot)
       /* perform_show */      {if   (objects->type [u] == object_ship1)
            /* show_ship_mark */      { /* perform_show_ship_mark */      {w_land->set_color (red);
      ...
      else  /* show_man_mark */      {w_land->set_color (red);
...
void player::show_mark (int u, int wx, int wy)
  {if (! is_robot)
       /* perform_show */      {if   (objects->type [u] == object_ship1)
            /* show_ship_mark */      {w_land->set_color (red);
      w_land->box       (wx, wy, wx + pic_dx*3-1, wy + pic_dy*3-1);
     }
      else  /* show_man_mark */      {w_land->set_color (red);
      w_land->box       (wx, wy, wx + pic_dx-1, wy + pic_dy-1);
...
void player::show_overview ()
  {if (! is_robot)
       /* perform_show */      {for (int x = 0; x < landscape_dx; x++)
...
void player::show_overview (int lx, int ly)
  {if (! is_robot &&  /* inside */ 
     (0 <= lx && lx < landscape_dx && 0 <= ly && ly < landscape_dy))
       /* perform_show */      {if   ( /* sunny */      (! is_suny || sun_cnt [lx][ly] > 0) ||  /* at_mine */      (unit [lx][ly] != none && objects->type [unit [lx][ly]] == object_mine))
            /* show_field */      {int xx     = lx * olandscale;
      int yy     = ly * olandscale;
      int obj_id = unit [lx][ly];
 
       /* show_landscape */      {int o_color;

       /* get_color */      {if   ( /* at_building */      (obj_id != none && is_building (objects->type [obj_id])) && objects->type [obj_id] != object_mine)
           o_color = objects->color [obj_id];
      else o_color =  /* actual_overview_color */      land_properties [landscape [lx][ly]].overview_color;
     };
      w_overview->set_color (o_color);
      w_overview->pixel     (xx,   yy);
     };
       /* show_objects */      {int obj_id = unit [lx][ly];
     ...
     (obj_id                  != none         &&
      ...
      objects->wood  [obj_id] == 1))
          /* perform_object_show */      {if (objects->with_overview [obj_id])
          /* show_obj_color */      {int o_color = objects->color [obj_id];
      ...
      else  /* show_black */      {int xx = lx * olandscale;
      int yy = ly * olandscale;
...
void player::show_overview_frame (int lx, int ly)
  {if (! is_robot)
       /* perform_show */      {int xx = lx * olandscale;
      int yy = ly * olandscale;
...
void player::show_overview_frame (bool is_display)
  {if (! is_robot)
       /* perform_show */      {if   (is_display)
            /* perform_display */      {for (int x = x0; x < x0 +  /* dxx */ (wdx / pic_dx); x++)
      ...
      else  /* perform_clear */      {for (int x = x0; x < x0 +  /* dxx */ (wdx / pic_dx); x++)
        {show_overview (x, y0);
  ...
  {if      (unit [mx][my] != none)
      ...
      for (int i = 0; i < max_objects; i++)
        ...
        {case object_worker        :
         ...
         case object_ship1         :
         case object_cata          :
         case object_doktor        :  /* grab_it */      {id        = i;
      best_dist = d;
     }; break;
        };
     };
...
void player::get_cmds (bool &is_quit,
  ...
  {if (! is_robot)
      ...
      w_unit    ->tick       ();
       ...
       /* handle_msg */      {g_msg->eval ();
      if (strlen (message) > 20)
         strcpy (message, substring (message, 0, 19));
      g_msg->refresh ();
     };
       /* handle_color_info */      {if (w_land->on ())
          /* check_color_info */ 
     {int xe;
      int ye;
      int b;
      ...
      if ( /* coords_valid */      (0 <= mx && mx <= landscape_dx && 0 <= my && my <= landscape_dy) && (u != last_unit || mx != last_mx || my != last_my))
         { /* perhaps_extra_mark */      {if (extra_mark_dx > 0)
          /* handle_extra_mark */      {for (int yy = extra_y; yy < extra_y + extra_mark_dy; yy++)
      ...
      for (int yy = extra_y; yy < extra_y + extra_mark_dy; yy++)
        for (int xx = extra_x; xx < extra_x + extra_mark_dx; xx++)
          show (mx, my, xx, yy, true);
           ...
           /* exec_color_info */      {bool was_on_unit = on_unit;
      ...
      if      (on_unit)      /* show_unit_color */      {if   (objects->color [u] == color)
            /* show_own_color */       {if   (objects->master [u])
      ...
      else if (was_on_unit)  /* clear_unit_color */      {act_cursor = cur_point_to;
      w_land->set_cursor (act_cursor);
    };
      last_unit = u;
      last_mx   = mx;
      last_my   = my;
       ...
       /* handle_marking */      {int xe;
      int ye;
      int b;

      if (running_cmd == none && w_land->is_mouse (xe, ye, b))
          /* handle_mark_event */      {if      ( /* is_new_marking */      (b == button1press))  /* try_to_mark_new */      {int mx;
      int my;
      ...
      my = i_bound (0, my, landscape_dy - 1);
       ...
       /* perhaps_release_old_mark */      {if (num_marked [act] > 0)
          /* release_all_marked */      {for (int m = 0; m < num_marked [act]; m++)
        if (! objects->is_free [marked [act][m]])
            /* release_marked_obj */      {int u = marked [act][m];

      objects->is_marked [u] = false;
      if (objects->type[u] == object_ship1)
         { /* refresh_ship */      {for (int xa = 0; xa < 3; xa++)
      ...
      num_marked [act] = 0;
       ...
       /* perhaps_set_new_mark */      {int u = point_to (mx, my);

      if (u != none)
          /* handle_mn */      {if  (objects->color [u] == color || objects->type [u] == object_mine)
            /* set_new_mark */      {marked [act][num_marked [act]] = u;
      ...
      else  /* write_ower */      {clear_status ();
      show_status  (u, true);
      if (objects->type [u] == object_ship1)   
          /* status_a_ship */      {int oc = objects->color [u];
      ...
      objects->color [u]       = oc;
       ...
       /* land_skip_left */      {int d;

      while (w_land->mouse (d, d, d))
        {};
     };
     }
      else if ( /* is_add_marking */      (b == button2press))  /* try_to_add_mark */      {int mx;
      int my;
      ...
      my = i_bound (0, my, landscape_dy - 1);
       ...
       /* handle_mark_toggle */      {int u = point_to (mx, my);

      if (u != none)
          /* handle_m */      {if   (objects->color [u]==color && objects->type [u]!=object_mine)
            /* perform_toggle */      {bool allready_marked;
      int  mi;    
      ...
      num_marked [act] = 0;
       ...
       /* check_allready_marked */      {allready_marked = false;
      ...
      if      (allready_marked)
               /* remove_from_marked */      { /* perform_remove */      {objects->is_marked [u] = false;
      if (objects->type[u] == object_ship1)
         { /* refresh_ship */      {for (int xa = 0; xa < 3; xa++)
        for (int ya = 0; ya < 3; ya++)
          show (objects->x [u]+xa, objects->y [u]+ya);
      ...
      if (num_marked [act] == 0)
         clear_status ();
               ...
               /* add_to_marked */      {marked [act][num_marked [act]] = u;
      if (objects->type [u] == object_ship1)
         num_marked_ships [act]++;
      objects->is_marked [u]         = true;
      num_marked [act]++;
      ...
      else if ( /* is_center */      (b == button3press))       /* center_screen */      {int nx;
      int ny;
      int mx;
      int my;
      ...
      if ( /* any_change */      (nx != x0 || ny != y0))
          /* perform_overview_move */      {show_overview_frame (false);
      x0 = nx;
      y0 = ny;
      show                ();
      show_overview_frame (true);
      ...
      while (w_land->mouse (d, d, d))
       ...
       /* perhaps_restatus */      {int uu = marked [act][0];
      ...
      if      (num_marked [act] > 1 && any_kill)
               /* handle_group_marking */      {for (int mi = 0; mi < num_marked [act]; mi++)
        { /* check_g_member */      {int u = marked [act][mi];
      ...
      if (objects->is_free [u]                      ||
          ...
          /* perform_remove */      {objects->is_marked [u] = false;
      if (objects->type[u] == object_ship1)
         { /* refresh_ship */      {for (int xa = 0; xa < 3; xa++)
        for (int ya = 0; ya < 3; ya++)
          show (objects->x [u]+xa, objects->y [u]+ya);
      ...
      if      (cmd_refresh_forced)
               /* perform_cmd_refresh */      {clear_status ();
      if      (num_marked [act] > 1)
              adjust_cmds ();
      ...
      cmd_refresh_forced = false;
       ...
       /* handle_cmds */      {num = 0;
       /* handle_cmd_buttons */      {char cc [2];

      cc [1] = 0;
      if      ((cc [0] = w_land->inchar ()) != 0)
               /* handle_key_cmd */      {switch (cc [0])
        {case '1'  :  /* handle_unit1_s */      {int i = 0;
       /* handle_new_unit_store */      {i++;
      num_marked       [i] = num_marked       [act];
      num_marked_ships [i] = num_marked_ships [act];
      marked_version   [i] = marked_version   [act];
       /* release_unit_buttons */      {int k;
      ...
      for (int j = 0; j < num_marked [act]; j++)
        marked [i][j] = marked [act][j];
         ...
         case '5'  :  /* handle_unit1_l */      {int i = 0;
       /* handle_new_unit_load */      {i++;
       /* unall_marked */      {for (int m = 0; m < num_marked [act]; m++)
        { /* single_unmarking */      {int u = marked [act][m];
      ...
      for (int j = 0; j < num_marked [act]; j++)
       ...
       /* do_remarking */      {for (int m = 0; m < num_marked [act]; m++)
        { /* single_remarking */      {int u = marked [act][m];
      int mi;
      ...
      if      (num_marked [act] == 1)
         ...
         default   :  /* men_cmd */      {for (int c = 0; c < num_cmds; c++)
        if (cmd_char [c] == cc [0])
           { /* handle_cmd_button */      {if (running_cmd != none)
          /* cancel_cmd */      {if (cmd_active [running_cmd])
      ...
      switch (cmd_code [c])
        {case cmd_sail         : 
         ...
         case cmd_guard        :
         case cmd_dig_row      : 
         case cmd_heap_row     :  /* get_x_y_param */      {running_cmd = c;
      ...
      cmds [running_cmd]->press (true);
         ...
         case cmd_built_uni    :  /* get_x_y_built */      {if   (cmd_code [c] == cmd_built_docks)
           set_extra_mark (3, 3);
      else set_extra_mark (2, 2);
       /* get_x_y_param */      {running_cmd = c;
      w_land->set_cursor (cur_focus);
      act_cursor = cur_focus;
      last_mx = -1;
      cmds [running_cmd]->press (true);
         ...
         case cmd_harvest      :  /* get_x_y_param */      {running_cmd = c;
      w_land->set_cursor (cur_focus);
      act_cursor = cur_focus;
      last_mx = -1;
      cmds [running_cmd]->press (true);
         ...
         case cmd_talk         :
         case cmd_train_pawn   :  /* store_cmd */      {cmds [c]->press (true);
      running_cmd = cmd_code [c];
       /* store_short_cmd */      {for (int m = 0; m < num_marked [act]; m++)
         /* store_short_unit_cmd */      {cmd    [m] = running_cmd;
      ...
      cmds [c]->press (false);
         ...
         case cmd_stop         :  /* handle_stop */      {cmds [c]->press (true);
      w_status->tick ();
      ...
      cmds [c]->press (false);
         ...
         case cmd_idle         :  /* handle_idle */      {cmds [c]->press (true);
      w_status->tick ();
      ...
      cmds [c]->press (false);
            ...
            break;
               ...
               /* check_buttons */      {for (int c = 0; c < num_cmds; c++)
         /* check_button */      {if (cmd_active [c] && cmds [c]->eval ())
         { /* handle_cmd_button */      {if (running_cmd != none)
          /* cancel_cmd */      {if (cmd_active [running_cmd])
         cmds [running_cmd]->press (false);
      running_cmd = none;
      ...
      if (running_cmd != none)
          /* handle_cmd_params */      {int xe;
      int ye;
      int b;

      if (w_land->is_mouse (xe, ye, b))
          /* check_store_or_abort */      {if      ( /* submit_pressed */      (b == button1press))
               /* store_cmd_params */      {int  mx;
      int  my;
      bool pos_ok;
      ...
      my = i_bound (0, my, landscape_dy - 1);
       ...
       /* check_pos_ok */      {switch (cmd_code [running_cmd])
       {case cmd_attack :  /* look_for_enemy */      {int u = point_to (mx, my);
      ...
      if (pos_ok && u != none)
        ...
        case cmd_trade  :  /* look_for_trade_point */      {pos_ok = (landscape [mx][my] == land_t_gold ||
                landscape [mx][my] == land_t_wood);
     }; break;
        default         :  /* look_at_field */      {pos_ok = true;
     };        break;
       };
     };
      if (pos_ok)
          /* grab_pos */      { /* store_unit_cmds */      {for (int m = 0; m < num_marked [act]; m++)
         /* store_unit_cmd */      {cmd    [m] = cmd_code [running_cmd];
      cmd_id [m] = marked [act][m];
      cmd_x  [m] = mx;
      cmd_y  [m] = my;
      ...
      if (cmd [m] == cmd_concentrate)
          /* handle_concentrate */      {if   (objects->type [cmd_id [m]] == object_ship1)
           cmd [m] = cmd_idle;
      ...
      if (cmd_active [running_cmd])
         cmds [running_cmd]->press (false);
       /* perhaps_release */      {switch (cmd_code [running_cmd])
        {case cmd_built_camp   : 
         case cmd_built_farm   : 
         case cmd_built_market : 
         case cmd_built_tents  : 
         ...
         case cmd_built_smith  : 
         case cmd_built_docks  : 
         case cmd_built_uni    :  /* release_all_marked */      {for (int m = 0; m < num_marked [act]; m++)
        if (! objects->is_free [marked [act][m]])
            /* release_marked_obj */      {int u = marked [act][m];
      ...
      set_extra_mark (0, 0);
       ...
       /* handle_dials */      {if (is_dial)
          /* read_limits */      {int m0 = marked [act][0];

      w_limit->eval (objects->wood_limit  [m0]);
      m_limit->eval (objects->money_limit [m0]);
      if (limit_zero->eval ())
          /* handle_limit_press */      {if   (objects->money_limit [m0] != 0 || objects->wood_limit [m0] != 0)
      ...
      m_limit->set (objects->money_limit [m0]);
       ...
       /* handle_overview_move */      {int xe;
      int ye;
      int b;

      if (w_overview->is_mouse (xe, ye, b))
          /* handle_overview */      {int xx;
      int yy;
      int bb;
      int nx;
      int ny;

       /* get_mouse */      {while (w_overview->is_mouse (xe, ye, b))
      ...
      show_overview_frame (true);
       ...
       /* handle_quit */      {if (b_quit->eval ())
          /* handle_quit_pressed */      {b_quit->press (true);
      ...
      if (is_debug && b_debug->eval ())
       ...
       /* handle_admin */      {if (money != last_money)
          /* show_money */      {char a [128];
      ...
      last_food = food;
       ...
       /* handle_pause */      {w_pause->mark_mouse ();
      w_pause->tick       ();
      ...
      if      (num_marked [act] == 1)
       ...
       /* handle_ship */      {int u = marked [act][0];

      if (u != none && objects->type [u] == object_ship1)
         {if  (objects->is_marked [u]) 
               /* get_ship_cmds */      {if   (num_marked_ships [act] == 1)
           objects->s [u]->eval ();
      else objects->s [u]->unshow (color);
     }
         else { /* refresh_ship */      {for (int xa = 0; xa < 3; xa++)
        for (int ya = 0; ya < 3; ya++)
          show (objects->x [u]+xa, objects->y [u]+ya);
     };
               objects->s [u]->unshow (color);
...
void player::mark (int id, bool mode)
  {if (objects->is_marked [id] != mode)
       /* perform_remarking */      {objects->is_marked [id] = mode;
      if   (mode)
            /* handle_add */      {marked     [act][num_marked [act]] = id;
      num_marked [act]++;
      if (objects->type [id] == object_ship1)
         num_marked_ships [act]++;
     }
      else  /* handle_remove */      {int i;

       /* get_i */      {for (i = 0; i < num_marked [act]; i++)
        if (marked [act][i] == id)
           break;
     };
...
void player::show_percent (int x, int y, int dx, int dy, int p, const char title [])
  {if (! is_robot)
       /* perform_show */      {int l1 = (int) ((double) dx * double (p) / 100.0);
      int l2 = dx - l1;
...
void player::clear_percent (int x, int y, int dx, int dy)
  {if (! is_robot)
       /* perform_show */      {w_status->set_color (gray80);
...
void player::clear_status ()
  {if (! is_robot)
       /* perform_show */      { /* clear_cmds */      {for (int i = 0; i < num_cmds; i++)
        if (cmd_active [i])
           {delete (cmds [i]);
            cmd_active [i] = false;
           }
...
void player::push_cmd (const char name [], int code, char c, int a_price, int a_wood)
  {cmds       [num_cmds] = new button (w_status, name,  /* x_cmd */  20,  /* y_cmd */  140 + num_cmds * 24);
   ...
   strcpy (bname [num_cmds], name);
    /* perhaps_write_price */      {if (a_price != 0 || a_wood != 0)
          /* write_price */      {char p [128];
...
bool supports (int type, int c)
  {switch (type)
     {case object_worker        :  /* worker */      {return (c == cmd_move_to      || 
              c == cmd_talk         ||
              c == cmd_attack       ||
              c == cmd_concentrate  ||
              c == cmd_dig          ||
      ...
      case object_knight        :  /* knight */      {return (c == cmd_move_to ||
              c == cmd_talk         ||
              c == cmd_concentrate ||
              c == cmd_attack  ||
              c == cmd_guard   ||
      ...
      case object_trader        :  /* trader */      {return (c == cmd_move_to      || 
              c == cmd_talk         ||
              c == cmd_concentrate  ||
              c == cmd_trade        ||
              c == cmd_idle);
    ...
    {return false;
      ...
      case object_camp          :  /* camp */     {return (c == cmd_train_knight ||
             c == cmd_idle);
    };          break;
      case object_farm          :  /* farm */     {return false;
    };          break;
      case object_market        :  /* market */     {return (c == cmd_train_trader ||
             c == cmd_idle);
    };        break;
      case object_tents         :  /* tents */     {return (c == cmd_train_scout ||
             c == cmd_idle);
    };         break;
      case object_mill          :  /* mill */     {return (c == cmd_train_archer ||
             c == cmd_idle);
    };          break;
      case object_smith         :  /* smith */     {return (c == cmd_train_cata ||
             c == cmd_idle);
    };         break;
      case object_docks         :  /* docks */     {return (c == cmd_built_ship  ||
             c == cmd_built_bship ||
             c == cmd_idle);
    };         break;
      case object_uni           :  /* uni */     {return (c == cmd_train_doktor ||
             c == cmd_idle);
    };           break;
      case object_archer        :  /* archer */      {return (c == cmd_move_to ||
              c == cmd_talk         ||
              c == cmd_concentrate ||
              c == cmd_attack  ||
              c == cmd_sad     ||
      ...
      case object_pawn          :  /* pawn */      {return (c == cmd_move_to ||
              c == cmd_talk         ||
              c == cmd_concentrate ||
              c == cmd_attack  ||
              c == cmd_guard   ||
      ...
      case object_scout         :  /* scout */      {return (c == cmd_move_to     ||
              c == cmd_talk         ||
              c == cmd_concentrate ||
              c == cmd_hide        ||
              c == cmd_idle);
     };         break;
      case object_ship1         :  /* ship1 */      {return (c == cmd_sail   ||
              c == cmd_attack ||
              c == cmd_enter  ||
              c == cmd_idle);
     };         break;
      case object_cata          :  /* cata */      {return (c == cmd_move_to ||
              c == cmd_talk         ||
              c == cmd_concentrate ||
              c == cmd_attack  ||
              c == cmd_fad     ||
      ...
      case object_doktor        :  /* doktor */      {return (c == cmd_move_to     ||
              c == cmd_talk         ||
              c == cmd_attack      ||
              c == cmd_concentrate ||
              c == cmd_heal        ||
  ...
  {bool is_ok = true;
    ...
    /* check_units */ 
     {for (int m = 0; m < num_marked [act]; m++)
         /* check_cmds */      {if (! objects->is_free [marked [act][m]])
          /* handle_unit */      {int id = marked [act][m];
...
void player::show_status (int id, bool is_first_time)
  {if (! is_robot)
       /* perform_show */      {if   (id == none)
            /* handle_multi_status */      {if (is_first_time)
          /* show_commands */      {switch (objects->type [id])
        {case object_worker :  /* show_worker_cmds */      {bool any_built_cmd;

       /* check_built_cmd */      {any_built_cmd = ((objects->cmd         [id][2] == cmd_harvest  ||
                        objects->cmd         [id][1] == cmd_harvest  ||
                        objects->cmd         [id][0] == cmd_harvest) &&
                       objects->harvest_type [id]    == harvest_built);
     };
      if   (any_built_cmd)
            /* push_worker_built */      {push_cmd ("Interrupt", cmd_idle,   'i',     0,           0);
      push_cmd ("  built  ", cmd_idle,   ' ',     0,           0);
     }
      else  /* push_worker_none_built */      {push_cmd ("Move       ", cmd_move_to,     'm', 0,           0);
      ...
      else  /* pawn_built_more */     {if ((num_town_halls < 2 && town_hall_in_progress == 0) ||
     ...
     push_cmd ("Uni        ", cmd_built_uni,    'u',price_uni,   wood_uni);
      ...
      push_cmd ("Idle       ", cmd_idle,        'i', 0,           0);
         ...
         case object_home   :  /* show_home_cmds */      {push_cmd ("Worker", cmd_train_worker, 'w', price_worker, 0);
      push_cmd ("Pawn  ", cmd_train_pawn,   'p', price_pawn,   wood_pawn);
      push_cmd ("Idle  ", cmd_idle,         'i', 0,            0);
     };   break;
         case object_market :  /* show_market_cmds */      {push_cmd ("Idle    ", cmd_idle,         'i', 0,            0);
     }; break;
         case object_tents  :  /* show_tents_cmds */      {push_cmd ("Idle    ", cmd_idle,         'i', 0,            0);
     };  break;
         case object_camp   :  /* show_camp_cmds */      {push_cmd ("Knight", cmd_train_knight, 'k', price_knight, wood_knight);
      push_cmd ("Idle  ", cmd_idle,         'i', 0,            0);
     };   break;
         case object_mill   :  /* show_mill_cmds */      {push_cmd ("Archer", cmd_train_archer, 'a', price_archer, wood_archer);
      push_cmd ("Idle  ", cmd_idle,         'i', 0,            0);
     };   break;
         case object_uni    :  /* show_uni_cmds */      {push_cmd ("Scientist", cmd_train_doktor, 's', price_doktor, wood_doktor);
      push_cmd ("Idle     ", cmd_idle,         'i', 0,            0);
     };    break;
         case object_smith  :  /* show_smith_cmds */      {push_cmd ("Catapult", cmd_train_cata, 'c', price_cata, wood_cata);
      push_cmd ("Idle    ", cmd_idle,       'i', 0,            0);
     };  break;
         case object_docks  :  /* show_docks_cmds */      {push_cmd ("Ship  ", cmd_built_ship,  's', price_ship1, wood_ship1);
/*
      push_cmd ("Battle", cmd_built_bship, 'b', price_ship2, wood_ship2);
*/
      push_cmd ("Idle  ", cmd_idle,        'i', 0,           0);
     };  break;
         case object_knight :  /* show_knight_cmds */      {push_cmd ("Move       ", cmd_move_to,     'm', 0,           0);
      push_cmd ("Concentrate", cmd_move_to,     'c', 0,           0);
      ...
      push_cmd ("Idle       ", cmd_idle,        'i', 0,           0);
         ...
         case object_trader :  /* show_trader_cmds */      {push_cmd ("Move       ", cmd_move_to,     'm', 0,           0);
      push_cmd ("Concentrate", cmd_move_to,     'c', 0,           0);
      push_cmd ("trade J    ", cmd_trade,       'j', 0,           0);
      push_cmd ("Talk (/)   ", cmd_talk,        '/', 0,           0); 
      push_cmd ("Idle       ", cmd_idle,        'i', 0,           0);
     }; break;
         case object_pawn   :  /* show_pawn_cmds */      {push_cmd ("Move       ", cmd_move_to,     'm', 0,           0);
      push_cmd ("Concentrate", cmd_move_to,     'c', 0,           0);
      push_cmd ("Attack     ", cmd_attack,      'a', 0,           0);
      push_cmd ("S & d      ", cmd_sad,         's', 0,           0);
      push_cmd ("F & d      ", cmd_fad,         'f', 0,           0);
      ...
      push_cmd ("Idle       ", cmd_idle,        'i', 0,           0);
         ...
         case object_scout  :  /* show_scout_cmds */      {push_cmd ("Move       ", cmd_move_to,     'm', 0,           0);
      push_cmd ("Concentrate", cmd_move_to,     'c', 0,           0);
      push_cmd ("Hide (A)   ", cmd_hide,        'a', 0,           0);
      push_cmd ("Talk (/)   ", cmd_talk,        '/', 0,           0); 
      push_cmd ("Idle       ", cmd_idle,        'i', 0,           0);
     };  break;
         case object_ship1  :  /* show_ship_cmds */      {w_status->set_color (30);
      ...
      push_cmd ("Idle       ", cmd_idle,        'i', 0,           0);
         ...
         case object_archer :  /* show_archer_cmds */      {push_cmd ("Move       ", cmd_move_to,     'm', 0,           0);
      push_cmd ("Concentrate", cmd_move_to,     'c', 0,           0);
      push_cmd ("Attack     ", cmd_attack,      'a', 0,           0);
      push_cmd ("S & d      ", cmd_sad,         's', 0,           0);
      push_cmd ("F & d      ", cmd_fad,         'f', 0,           0);
      ...
      push_cmd ("Idle       ", cmd_idle,        'i', 0,           0);
         ...
         case object_doktor :  /* show_doktor_cmds */      {push_cmd ("Move       ", cmd_move_to,   'm', 0,           0);
      push_cmd ("Concentrate", cmd_move_to,   'c', 0,           0);
      ...
      push_cmd ("Idle       ", cmd_idle,      'i', 0,           0);
         ...
         case object_cata   :  /* show_archer_cmds */      {push_cmd ("Move       ", cmd_move_to,     'm', 0,           0);
      push_cmd ("Concentrate", cmd_move_to,     'c', 0,           0);
      push_cmd ("Attack     ", cmd_attack,      'a', 0,           0);
      push_cmd ("S & d      ", cmd_sad,         's', 0,           0);
      push_cmd ("F & d      ", cmd_fad,         'f', 0,           0);
      ...
      else  /* handle_single_status */      { /* get_version */      {marked_version [act] = objects->version [id];
     };
       /* show_name */      {w_status->set_color (black);
      w_status->write     (30, 30, objects->name [id]);
     };
       /* show_status_data */      {if (objects->type [id] != object_mine)
          /* show_health */      {show_percent (30, 70, 100, 6, objects->health [id], "health");
     };
      switch (objects->type [id])
        {case object_worker        : 
         ...
         case object_archer        : 
         case object_doktor        : 
         case object_cata          :  /* show_man_status */      { /* show_health */      {show_percent (30, 70, 100, 6, objects->health [id], "health");
     };
       /* show_treasury */      { /* show_gold */      {char msg [128];

      sprintf             (msg, "Gold   : %d", objects->money [id]);
      w_status->set_color (gold3);
      w_status->write     (40, 100, msg);
     };
       /* show_wood */      {char msg [128];
      ...
      w_status->write     (40, 120, msg);
         ...
         case object_home          :  /* show_home_status */      { /* show_task */      {switch (objects->cmd [id][0])
        {case cmd_train_worker : 
         case cmd_train_knight : 
         case cmd_train_archer : 
         case cmd_train_trader : 
         ...
         case cmd_built_bship  :
         case cmd_train_scout  : 
         case cmd_train_cata   :  /* show_train_status */      {show_percent (30, 90, 100, 6, objects->power [id], "done");
     }; break;
         case cmd_idle         :  /* show_idle */      {clear_percent (12, 80, 100, 30);
     };         break;
        };
     };
     };   break;
         case object_farm          :  /* show_farm_status */      {char msg [128];
      ...
      w_status->write     (30, 120, msg);
         ...
         case object_market        :  /* show_market_status */      { /* show_task */      {switch (objects->cmd [id][0])
        {case cmd_train_worker : 
         case cmd_train_knight : 
         case cmd_train_archer : 
         case cmd_train_trader : 
       ...
       /* show_suport_status */      {if (objects->color [id] == color)
          /* display_free */      {char msg [128];

      sprintf             (msg, "Free : %d    ", objects->support [id]);
      w_status->set_color (black);
      w_status->write     (30, 120, msg);
      if (objects->type [id] != object_tents)
          /* show_limits */      {if (objects->color [id] == color)
          /* display_limits */      {is_dial= true;
      ...
      if (objects->money_limit [id] == 0 &&
         ...
         case object_tents         :  /* show_tents_status */      { /* show_task */      {switch (objects->cmd [id][0])
        {case cmd_train_worker : 
         case cmd_train_knight : 
         case cmd_train_archer : 
         case cmd_train_trader : 
      ...
      if (objects->money_limit [id] == 0 &&
         ...
         case object_camp          :  /* show_camp_status */      { /* show_task */      {switch (objects->cmd [id][0])
        {case cmd_train_worker : 
         case cmd_train_knight : 
         case cmd_train_archer : 
         case cmd_train_trader : 
      ...
      if (objects->money_limit [id] == 0 &&
         ...
         case object_uni           :  /* show_uni_status */      { /* show_task */      {switch (objects->cmd [id][0])
        {case cmd_train_worker : 
         case cmd_train_knight : 
         case cmd_train_archer : 
         case cmd_train_trader : 
      ...
      if (objects->money_limit [id] == 0 &&
         ...
         case object_mill          :  /* show_mill_status */      { /* show_task */      {switch (objects->cmd [id][0])
        {case cmd_train_worker : 
         case cmd_train_knight : 
         case cmd_train_archer : 
         case cmd_train_trader : 
      ...
      if (objects->money_limit [id] == 0 &&
         ...
         case object_smith         :  /* show_smith_status */      { /* show_task */      {switch (objects->cmd [id][0])
        {case cmd_train_worker : 
         case cmd_train_knight : 
         case cmd_train_archer : 
         case cmd_train_trader : 
      ...
      if (objects->money_limit [id] == 0 &&
         ...
         case object_docks         :  /* show_docks_status */      { /* show_task */      {switch (objects->cmd [id][0])
        {case cmd_train_worker : 
         case cmd_train_knight : 
         case cmd_train_archer : 
         case cmd_train_trader : 
         ...
         case object_building_site :  /* show_train_status */      {show_percent (30, 90, 100, 6, objects->power [id], "done");
     };  break;
         case object_site_docks    :  /* show_train_status */      {show_percent (30, 90, 100, 6, objects->power [id], "done");
     };  break;
         case object_mine          :  /* show_mine_status */      { /* show_gold */      {char msg [128];
      ...
      w_status->write     (40, 100, msg);
         ...
         case object_ship1         :  /* show_ship_status */      {if (objects->color [id] == color)
...
void player::edit ()
  {menu *edit_men;
   ...
   int  last_y;
    ...
    /* init_edit */      { /* open_edit_men */      {edit_men = new menu ("edit_men",
                           by_fix,
                           by_fix,
    ...
    /* perform_edit */      {while (! quit)
        { /* edit_cycle */      { /* init */      {w_land    ->mark_mouse (); 
      w_land    ->tick       ();
      w_overview->mark_mouse ();
      w_overview->tick       ();
     };
      ...
      if ((cmd = edit_men->eval ()) != no_menu_request)
          /* exec_cmd */      {edit_men->release ();
       /* set_button */      {if (cmd < 8)
          /* set_buttons */      { /* release_buttons */      {for (int i = 0; i < 8; i++)
        edit_men->release (i);
     };
      edit_men->press (cmd);
     };
      ...
      switch (cmd)
        {case 0 : act = land_grass; break;
         ...
         case 8 :  /* handle_save */      {save_land (land_name);
     };      break;
      ...
      last_y = my;
      switch (act)
        {case land_grass :  /* set_grass */      { /* check_field */      {int u = point_to (mx, my);
      ...
      landpic   [mx][my] = land_grass;
         ...
         case land_water :  /* set_water */      {if (landhight [mx][my] <= 0)
         { /* check_field */      {int u = point_to (mx, my);
      ...
      if (u != none)
         ...
         case land_bush  :  /* set_bush */      { /* check_field */      {int u = point_to (mx, my);
      ...
      landpic   [mx][my] = land_bush;
         ...
         case land_wood  :  /* set_wood */      { /* check_field */      {int u = point_to (mx, my);
      ...
      landpic   [mx][my] = land_wood;
         ...
         case 1001       :  /* set_mine */      { /* check_field */      {int u = point_to (mx, my);
      ...
      objects->create_building (mx, my, object_mine, 10000, 0 , none);
         ...
         case 1002       :  /* set_plus */      {objects->readjust_land (mx, my, 1);
     };  break;
         case 1003       :  /* set_minus */      {objects->readjust_land (mx, my, -1);
     }; break;
         case 1004       :  /* set_home */      { /* check_field */      {int u = point_to (mx, my);
      ...
      w_land->mouse (d, d, d);
       ...
       /* handle_scrollers */      {int xe;
      int ye;
      int b;

      if (w_land->is_mouse (xe, ye, b) &&  /* scroller_event */      (b == button3press))
          /* handle_scroller */      {int xx;
      int yy;

      w_land->mouse (xe, ye, xx, yy, b);
      if (xe <= pic_dx)        /* handle_left */      {show_overview_frame (false);
      x0 = i_max (0, x0 - 1);
      show                ();
      show_overview_frame (true);
     };
      if (xe >= wdx - pic_dx)  /* handle_right */      {show_overview_frame (false);
      x0 = i_min ( /* max_x */  (landscape_dx - (wdx / pic_dx)), x0 + 1);
      show                ();
      show_overview_frame (true);
     };
      if (ye <= pic_dy)        /* handle_up */      {show_overview_frame (false);
      y0 = i_max (0, y0 - 1);
      show                ();
      show_overview_frame (true);
     };
      if (ye >= wdy - pic_dy)  /* handle_down */      {show_overview_frame (false);
...
void player::add_sun (int lx, int ly, int r)
  {if (is_suny && ! is_robot && active)
       /* perform_sun */      {for (int x = i_max (0, lx-r); x <= i_min (landscape_dx-1, lx+r); x++)
...
void player::move_sun_d (int lx, int ly, int dx, int dy, int r)
  {if (is_suny && ! is_robot && active)
       /* perform_sun */      {int ix = i_sign (dx); 
      int iy = i_sign (dy);

       /* perform_sub */      {if (iy != 0)  /* sub_y */      {int y = ly - (r * iy);
  
      if (0 <= y && y <= landscape_dy-1)
          /* perform_sub_y */      {for (int x = i_max (0, lx-r); x <= i_min (landscape_dx-1, lx+r); x++)
        { /* sub */      {sun_cnt [x][y]--;
      if (sun_cnt [x][y] == 0)
         {show          (x, y);
          show_overview (x, y);
      ...
      if (ix != 0)  /* sub_x */      {int x = lx - (r * ix);

      if (0 <= x && x <= landscape_dx-1)
          /* perform_sub_x */      {for (int y = i_max (0, ly-r); y <= i_min (landscape_dy-1, ly + r); y++)
        { /* sub */      {sun_cnt [x][y]--;
      if (sun_cnt [x][y] == 0)
         {show          (x, y);
          show_overview (x, y);
       ...
       /* perform_add */      {if (iy != 0)  /* add_y */      {int y = ly + ((r+1) * iy);

      if (0 <= y && y <= landscape_dy-1)
          /* perform_add_y */      {for (int x=i_max (0, lx-(r)); x<=i_min (landscape_dx-1, lx+(r)); x++)
        { /* add */      {sun_cnt [x][y]++;
      if (sun_cnt [x][y] == 1)
         {show          (x, y);
          show_overview (x, y);
      ...
      if (ix != 0)  /* add_x */      {int x = lx + ((r+1) * ix);

      if (0 <= x && x <= landscape_dx-1)
          /* perform_add_x */      {for (int y=i_max (0, ly-(r)); y<=i_min (landscape_dy-1, ly+(r)); y++)
        { /* add */      {sun_cnt [x][y]++;
      if (sun_cnt [x][y] == 1)
         {show          (x, y);
          show_overview (x, y);
...
void player::handle_pause ()
  { /* update_buttons */      {for (int i = 0; i < num_players; i++)
        {if (p_pressed [i] != paused [i])
             /* redisplay_button */      {p_button [i]->press (paused [i]);
      p_pressed [i] = paused [i];
     };
        };
     };
    /* handle_own */      {w_pause->mark_mouse ();
      w_pause->tick       ();
      if (p_button [p_no]->eval ())
...
void player::set_extra_mark (int dx, int dy)
  {if      (dx == 0 && extra_mark_dx != 0)   /* switch_off */      {for (int yy = extra_y; yy < extra_y + extra_mark_dy; yy++)
        for (int xx = extra_x; xx < extra_x + extra_mark_dx; xx++)
          show (xx, yy);
      extra_mark_dx = dx;
      extra_mark_dy = dy;
     }
   else if (dx != 0 && extra_mark_dx == 0)   /* switch_on */      {extra_mark_dx = dx;
      extra_mark_dy = dy;
      extra_x       = last_mx;
      extra_y       = last_my;
      for (int yy = extra_y; yy < extra_y + extra_mark_dy; yy++)

...
void ship::hit(int idh, int power)
{
    ...
    { /* perform_man_hit */
        ...
        id = unit[i];
        if (0 <= id && id < max_objects && /* object_should_be_hit */ (!objects->is_free[id] && objects->type[id] != object_zombi && objects->type[id] != object_ship_zombi && objects->type[id] != object_schrott))
        /* handle_obj */ {

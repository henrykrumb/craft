...
void scroller::calc_ruler_params () {
    if   (dx > dy)
        ...
        ruler_x  = x +  /* hor_x0 */     (bar_button_size + 2) + (int) ( /* hor_scale */  ((double)  /* hor_space */  (dx - 4 - 2 * bar_button_size) / (double) (max - min)) * (double) (pos-min));
        ruler_y  = y + 2;
        ...
        ruler_y  = y +  /* vert_y0 */     (bar_button_size + 2) + (int) ( /* vert_scale */  ((double)  /* vert_space */  (dy - 4 - 2 * bar_button_size) / (double) (max - min)) * (double) (pos-min));
        ruler_x  = x + 2;
...
bool scroller::eval(int &v_pos) {
    /* handle_mouse_events */      {
        if (on ())
            /* handle_event */      {
            if      ( /* on_small_incr */      (button_incr->eval ())) { /* handle_small_incr_event */
                set (pos + step_small);
                was_event = true;
            } else if ( /* on_small_decr */      (button_decr->eval ())) { /* handle_small_decr_event */
                set (pos - step_small);
                was_event = true;
                ...
                if (button == button1press)
                    /* handle_move_event */      {
                    if   ( /* on_ruler */      (ruler_x <= xm && xm <= ruler_x + ruler_dx &&
                                                ruler_y <= ym && ym <= ruler_y + ruler_dy))
                        /* handle_ruler_event */      {
                        while ( /* mouse_hold_down */      ((w->mouse (xm, ym, button) || true) && button != button1release)) {
                            /* shift_ruler */      {
                                if   (dx > dy)
                                    /* horizontal_shift */      {
                                    if      (xm < ruler_x_center) {
                                        set (pos - 1);
                                    } else if (xm > ruler_x_center) {
                                        set (pos + 1);
                                    }
                                } else { /* vertical_shift */
                                    if      (ym < ruler_y_center) {
                                        set (pos - 1);
                                    } else if (ym > ruler_y_center) {
                                        set (pos + 1);
                                    }
                                };
                            };

...
void dump ()
  { /* print_landscape */      {printf ("--- landscape\n");
    ...
    /* print_landhight */      {printf ("--- landhight\n");
      printf ("dx %d dy %d\n", landscape_dx, landscape_dy);
      for (int y = 0; y < landscape_dy; y++)
        {for (int x = 0; x < landscape_dx; x++)
    ...
    /* print_objs */      {for (int i = 0; i < max_objects; i++)
...
void init_craft (char   host  [max_players][128],
  ...
  {install_abnormal_termination_handler ();
    /* store_params */      {num_players = 0;
      num_humans  = 0;
      for (int i = 0; i < max_players; i++)
         /* cnt_player */      {if (active [i])
         {num_players++; 
          if (!  /* is_robot */      (strcmp (host [i], "-") == 0))
             num_humans++;
         };
     };
      num_paused  = 0;
     };
    /* init_ticker */      {ticker = 0;
     };
    /* load_troups */      {objects = new object_handler ();
     };
    /* init_land */      {load_land_props ();
      if   (is_water_world)
           rnd_water_land  ();  
      else rnd_land ();
     };
    /* init_players */      {int i;
      ...
      for (i = 0; i < max_players; i++)
        if (active [i])
            /* gen_player */      {if (i == 0) players [i] = new player (i,
                                            name [i],
                                            host [i],
                                            red,
                                            p_speed [i],
                                             /* is_robot */      (strcmp (host [i], "-") == 0));
...
void set_initial_objects ()
   {int color = blue;
    int il_locked [15];
     ...
     /* init */      {for (int i = 0; i < 15; i++)
        il_locked [i] = false;
     };
    if (active [0]) {color = blue;    /* gen */      {int xx;
      int yy;

       /* get_xx_yy */      {if   (is_water_world)
            /* get_island_xx_yy */      {int il;

       /* get_il */      {il = i_random (0, land_num_il () - 4 - 1);
     };
      while (il_locked [il])
        { /* get_il */      {il = i_random (0, land_num_il () - 4 - 1);
     };
      ...
      bool il_ok;
       ...
       /* check_il */      {il_ok =  /* could_become_grass */      (1 <= xx && xx <= landscape_dx - 3 &&
      ...
      while (! il_ok)
        { /* perhaps_backstep */      {if (is_water (10000, xx, yy))
         {xx = ox;
          yy = oy;
         };
     };
      ...
      landscape [xx+1][yy+1] != land_sea);
       ...
       /* set_to_grass */      {for (int xa = i_max (0, xx-2); xa < i_min (landscape_dx, xx+3); xa++)
      ...
      for (int i = 0; i < max_objects; i++)
       ...
       /* gen_on_land */      {objects->create_worker (xx,     yy,  color, none);
      objects->create_worker (xx+1,   yy,  color, none);
      objects->create_knight (xx+1, yy+1,  color, none, is_self && !  /* is_robo */      players [color_player [color]]->is_robot);
      players [color_player [color]]->focus (xx, yy);
     };
     };};
    if (active [1]) {color = red;     /* gen */      {int xx;
      int yy;
    ...
    if (active [2]) {color = yellow;  /* gen */      {int xx;
      int yy;
    ...
    if (active [3]) {color = cyan;    /* gen */      {int xx;
      int yy;
...
int main (int num_params, char *shell_params [])
  {double act_speed;
    ...
    /* start_message */      {printf ("CRAFT Version 3.5 (http://set.gmd.de/~hua) started\n");
     };
    /* init_players */      {int tic;

       /* get_params */      {for (int i = 0; i < max_players; i++)
        active [i] = false;
   ...
   set_initial_objects ();
    /* exec_craft */      {bool quit = false;
      int  cnt;
      int  h = 0;

       /* set_cnt */      {cnt = num_players;
     };
      while (! quit)
         /* cycle */      {int    suspend = -1;
      double cycle_start_time = x_sys_time ();

       /* read_cmds */      {for (int pi = 0; pi < max_players; pi++)
     ...
     {int  cmd [max_marked];
      ...
      players [pi]->get_cmds (is_suspend, num, cmd, id, x, y);
       /* perhaps_exec_player_cmd */      {for (int p = 0; p < num; p++)
         /* exec_player_cmd */      {objects->new_order (id [p], cmd [p], x [p], y [p]);
     };
     };
      if (is_suspend)
         {suspend = pi;
           /* check_over */      {int only = -1;
      ...
      if      (players [i]->master_dead)
       ...
       /* tell_to_players */      {for (int j = 0; j < max_players; j++)
        {m_active [j] = false;
         if (active [j] && ! players [j]->is_robot)
            /* tell_to_player */      {tell (host [j], players [j]->w_land, w [j], msg);
      b        [j] = new button (w [j], "OK", 120, 80);
      m_active [j] = true;
     };
        };
     };
       /* ack_from_players */      {bool any_active = true;

      while (any_active)
         /* check_buttons */      {any_active = false;
      for (int j = 0; j < max_players; j++)
        if (m_active [j])
            /* ack_from_player */      {any_active = true;
      w [j]->mark_mouse ();
      w [j]->tick ();
      if   (b [j]->eval ())
           {delete (b [j]);
      ...
      if (h % 3 == 0)
          /* check_pause */      {bool was = false;

      while (num_paused > 0) 
        { /* pause_a_bit */      {was = true;
      for (int i = 0; i < max_players; i++)
      ...
      for (int i = 0; i < max_players; i++)
       ...
       /* exec_objects */      {for (int i = 0; i < 1; i++)
        {objects->exec ();
         ticker++;
        };
     };
      ...
      active [i] = false;
       ...
       /* perhaps_delay */      {int rest_dt = (int)(min_cycle_time - (x_sys_time () - cycle_start_time));

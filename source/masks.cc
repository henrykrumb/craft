...
void frame (win *w)
  {int dx             = w->dx ();
   ...
   int c_border_light = win_default_c ("frame_border_light");
    ...
    /* fill_background */      {w->function  (GXcopy);
      w->set_color (win_default_c ("frame_background"));
      w->fill      (0, 0, dx, dy);
      w->set_font  (frame_font);
     };
   for (int i = 0; i < frame_size; i++)
      /* draw_frame_line */      {w->set_color (c_border_light);
      w->line      (i, i, dx - i, i);
      w->line      (i, i, i, dy - i);
  ...
  {win    *w;
   ...
   bool   ok_pressed;
    ...
    /* open_mask */      {int dx;
      ...
      w->write          ( /* x_text */      (200 - dx) / 2, 40, question);
     };
    /* get_answer */      {bool answer   = false;
      int  event_id = -1;

      ok_pressed = false;
      while (! answer)
        { /* try_to_get_the_answer */      {w->mark_mouse ();
      event_id   = w->event_id;
      ok_pressed = ok->eval ();
...
void ack (const char host [], char message [])
  {win    *w;
   ...
   int    dy;
    ...
    /* calc_dx_dy */      {int num_lines;

      text_size (message, win_default_font, dx, dy, num_lines);
     };
    /* open_mask */      {w  = new win      ("janus:ack", host,  100, 100, dx + 40,dy + 100);
      frame             (w);
    ...
    /* write_message */      {char record [1024];
      int  y  = 20;
      int  rp = 0;

      for (int i = 0; i < strlen (message); i++)
         /* handle_char */      {if   ( message [i] == '\n')
            /* handle_new_record */      {int tdx;
      int tdy;

      record [rp] = 0;
       /* calc_tdx_tdy */      {w->text_size (record, tdx, tdy);
     };
      y += tdy + 10;
...
void tell (const char host [], win *&w, char message [])
  { /* open_mask */      {w = new win ("janus:tell", host, 100, 100, 300, 100);
      frame   (w);
      w->tick ();
     };
    /* tell_message */      {int dx;
      ...
      w->write          ( /* x_text */      (300 - dx) / 2,  /* y_text */      (100 - dy) / 2, message);
      w->tick           ();
     };
...
void tell (char host [], win *parent, win *&w, char message [])
  { /* open_mask */      {w = new win (parent, "janus:tell", host, 100, 100, 300, 100);
      frame   (w);
      w->tick ();
     };
    /* tell_message */      {int dx;
...
bool get_line (char s    [],
  ...
  {win     *w;
   ...
   bool    is_cancel;
    ...
    /* open_win */      {w      = new win (name, "", 100, 100, dx + 40, 80);
      background (w);
      gl     = new craft_getline (name, w, s, 20, 20, dx, 20, with_history);
      cancel = new button (w, "cancel", (dx+40) / 2 - 60, 50);
     };
    /* perform_get */      {is_cancel = false;
      while (! gl->eval () && ! is_cancel)
        { /* check_cancel */      {is_cancel = cancel->eval ();
     };
        };
     };
    /* delete_win */      {delete (gl);
      delete (cancel);
      delete (w);
     };
   return ! is_cancel;
...
int select (char menu_string [], char *name)
  {menu *m;
   ...
   int  selected;
    ...
    /* complete_m_string */      {m_string = new char [strlen (menu_string) + 30];
    ...
    /* open_m */      {m = new menu (m_name, by_fix, by_fix, m_string);
     };
    /* perform_select */      {while ((selected = m->eval ()) == no_menu_request)
        {
        };
     };
    /* adjust_selected */      {if (selected == m->num_buttons-1)
         selected = -1;
     };
   delete (m);
   delete[] (m_string);

...
void text_size (char s [], const char font [], int &dx, int &dy, int &num_lines)
  {XFontStruct *font_info;
   Display     *display;
    ...
    /* pharse_string */      {char record [1024];
      int  rp;

      rp = 0;
      for (int i = 0; i < strlen (s); i++)
         /* handle_char */      {if   (s [i] == '\n')
            /* handle_new_record */      {int tdx;
      int tdy;

      record [rp] = 0;
      num_lines++;
       /* calc_tdx_tdy */      {tdx = XTextWidth (font_info, record, strlen (record));
    ...
    /* close_font_info */      {XFreeFont     (display, font_info);
...
void init_default_handler ()
  {if (! win_handler_init)
       /* perform_init */      {win_handler_init = true;
...
win::win (const char title [],
  ...
  { /* perhaps_init_fix */      if (! fix_initialized)
         /* init_fix */      {fix_initialized = true;
      strcpy (fix_name, ".winfix");
      strcpy (fix_dir, "");
     };
   is_enable = enable;
   if (is_enable)
       /* perform_open */      {char                 full_title [128] = "win:";
      char                 full_host  [128];
      int                  argc = 0;
      char                 **argv = NULL;
      XSetWindowAttributes new_attr;

       /* init_mouse */      {num_mouse_events = 0;
      event_id         = 0;
      mouse_x          = 0;
      mouse_y          = 0;
      mouse_inside     = false;
      ...
      is_alien           = false;
       ...
       /* perhaps_user_position */      { /* perhaps_use_fixpos */      if (x == by_fix || y == by_fix)
        {fix_pos (x, y);
         with_fix = true;
        };
       /* perhaps_use_user_pos */      {if   (x != by_user && y != by_user)
            /* set_to_pos */      {myhint->flags |= PPosition;
      myhint->x     = x;
      myhint->y     = y;
      open_x       = x;
      open_y       = y;
     }
      else  /* set_to_external_position */      {/* myhint->flags |= USPosition; */
      myhint->x     = 400;
      myhint->y     = 400;
     };
     };
     };
       /* perhaps_user_size */      { /* perhaps_use_fixsize */      if (dx == by_fix || dy == by_fix)
        {fix_size (w_dx, w_dy);
         with_fix = true;
         myhint->flags |= PSize;
        };
       /* perhaps_use_user_size */      {if   (w_dx != by_user && w_dy != by_user)
            /* set_to_size */      {myhint->flags  |= PSize;
      myhint->width  = w_dx;
      myhint->height = w_dy;
      open_dx       = w_dx;
      open_dy       = w_dy;
     }
      else  /* set_external */      {myhint->flags |= USSize;
      myhint->width  = 10;
      myhint->height = 10;
     };
     };
     };
       /* perhaps_resize */      if (! resize_enable)
        {myhint->min_width  = myhint->width;
         myhint->max_width  = myhint->width;
         myhint->min_height = myhint->height;
         myhint->max_height = myhint->height;
        };
       /* create_window */       {mywindow = XCreateSimpleWindow (mydisplay,
      ...
      create_color_map ();
       /* set_window_attributes */      {new_attr.backing_store = Always;
      ...
      set_font                (win_default_font);
       ...
       /* raise_window */      {tick (false);
     };
       /* set_font_info */      {XGCValues gcinfo;
...
win::~win ()
  {if      (is_alien)
            /* close_alien */      {if (is_enable)
         XDestroyWindow (mydisplay, mywindow);
     }
   else if (is_enable)
            /* close_own */      { /* perhaps_fix */      {if (x  () != open_x  || y  () != open_y ||
  ...
  {bool first = true;
   ...
   if (is_enable)
      ...
      num_mouse_events            = i_min (max_mouse_events-1, 
        ...
        case ButtonPress    :  /* handle_mouse_button_press */      {switch (((XButtonPressedEvent *) &myevent)->button)
      ...
      num_mouse_events             = i_min (max_mouse_events-1,
        ...
        case MappingNotify  :  /* handle_notify */      XRefreshKeyboardMapping((XMappingEvent*)&myevent);               break;
        case KeyPress       :  /* handle_press_key */      {char instring [10];

      press_cnt++;
       /* decode_input */      {KeySym mykey;
      int    d;
      ...
      if (XKeysymToString(mykey))
       ...
       /* push_to_queue */      {inbuffer  [keycount] = instring [0];
      keybuffer [keycount] = act_key;
      keycount             = i_min (keybuf_size, keycount+1);
     };
       /* perhaps_skip_shift */      {if (act_key == 106)
         keycount--;
     };
     };            break;
        case KeyRelease     :  /* handle_release_key */      {press_cnt--;
     };          break;
        case EnterNotify    :  /* handle_enter */      {mouse_inside = true;
     };                break;
        case LeaveNotify    :  /* handle_leave */      {mouse_inside = false;
     };                break;   
        case MotionNotify   :  /* send_motion_coords */     {mouse_x      = ((XMotionEvent *) &myevent)->x;
...
char win::inchar (int &key, char *cntl)
  {if   (keycount <= 0)
         /* handle_no_char */      {key      = 0;
      cntl [0] = 0;
      return 0;
     }
   else  /* get_top_of_queue */      {char c;

      c   = inbuffer  [0];
      key = keybuffer [0];
      strcpy (cntl, cntlbuffer [0]);
       /* shift_rest */      {delchar (inbuffer, 0);
...
bool win::mouse (int &x, int &y, int &xe, int &ye, int &button)
  {bool flag;
   ...
   if   (is_enable)
         /* handle_mouse */      {if   (num_mouse_events > 0)
            /* grab_event */      {num_mouse_events--;
      x      = mouse_x;
      y      = mouse_y;
      xe     = mouse_ex    [0];
      ye     = mouse_ey    [0];
      button = mouse_event [0];
       /* shift_buffer */      {for (int i = 0; i < max_mouse_events-1; i++)
...
bool win::is_mouse (int &xe, int &ye, int &button)
  {if   (is_enable && num_mouse_events > 0)
         /* handle_mouse */      {xe     = mouse_ex    [0];
      ye     = mouse_ey    [0];
      button = mouse_event [0];
      return true;
     }
   else  /* handle_no_mouse */      {xe = mouse_x;
      ye = mouse_y;
      return false;
     };
...
int win::win_color (int r, int g, int b)
  {if   ( /* same_color_as_last_time */      (r == last_r && g == last_g && b == last_b))
        return last_best_color;
   else  /* search_best_color */      {double best_dist;
      int    best_color;

       /* init */      {best_dist = DBL_MAX;
     };
       /* check_colors */      {for (int i = 1; i < max_colors; i++)
         /* check_color */      {int d;

       /* calc_d */      {int dr = color_r [i] - r;
      int dg = color_g [i] - g;
      int db = color_b [i] - b;
      ...
      if (d < best_dist)
          /* grab_color */      {best_dist  = d;
      best_color = i;
     };
     };
       /* check_black */      {int d = r*r + g*g + b*b;
      ...
      if (d < best_dist)
       ...
       /* check_white */      {int d = (255-r)*(255-r) + (255-g)*(255-g) + (255-b)*(255-b);
...
void win::set_color (int color)
  {if (is_enable)
        /* perform_set_color */      {unsigned  long c = colors [color];
      XGCValues gcval;
...
void win::function (int func) 
  {XGCValues gcval;
   ...
   if (is_enable)
       /* perform_function */      {gcval.function = func;
...
void win::pixel (int x, int y)
  {if (is_enable)
       /* perform_draw */      {XDrawPoint (mydisplay,
                  mywindow,
                  mygc,
                  x, y);
     };
...
void win::draw (polyline *p)
  {if (is_enable)
       /* perform_draw */      {XDrawLines (mydisplay,
                  mywindow,
                  mygc,
                  p->p,
                  p->n,
...
void win::line (int x1, int y1, int x2, int y2)
  {if (is_enable)
       /* perform_line */      {XDrawLine (mydisplay,
                 mywindow,
                 mygc,
                 x1, y1, x2, y2);
     };
...
void win::load_map (char name [], Pixmap &m, int &dx, int &dy)
  { /* check_file */      {if (! f_exists (name))
         errorstop (2, "WIN", "load_map with unknown map", name);
     };
   if (is_enable)
       /* perform_load */      {unsigned int dxx;
      unsigned int dyy;
               int xh;
               int yh;
...
void win::show_map (int x, int y, char name [])
  { /* check_file */      {if (! f_exists (name))
   ...
   if (is_enable)
       /* perform_show */      {         Pixmap p;
      unsigned int    dx;
      unsigned int    dy;
               int    xh;
               int    yh;
     
       /* read_pixmap */      {XReadBitmapFile (mydisplay, mywindow,
                       name,
                       &dx, &dy, &p, &xh, &yh);
     };  
       /* show_pixmap */      {XCopyPlane(mydisplay,
                 p, mywindow,
                 mygc,
                 0, 0, dx, dy, x, y, 1L);
     };
...
void win::show_map (int    x,     int y, 
                    ...
                    int    dx,    int dy)
  {if (is_enable)
       /* perform_show */      {unsigned int dxx = dx;
      unsigned int dyy = dy;
       /* show_pixmap */      {XCopyArea      (mydisplay,
                      m, 
                      mywindow,
                      mygc,
...
void win::show_map (Pixmap m,
  ...
  {if (is_enable)
       /* perform_show */      {XSetClipMask   (mydisplay,
                      mygc,
                      mask);
      XSetClipOrigin (mydisplay,
                      mygc,
...
void win::fill (polyline *p)
  {if (is_enable)
       /* perform_fill */      {XFillPolygon (mydisplay,
                    mywindow,
                    mygc,
                    p->p,
                    p->n,
...
void win::fill (int x1, int y1, int dx, int dy)
  {if (is_enable)
       /* perform_fill */      {XFillRectangle (mydisplay,
                      mywindow,
                      mygc,
                      x1, y1, dx, dy);
     };  
...
void win::clear ()
  {if (is_enable)
       /* perform_clear */      {fill (0, 0, w_dx, w_dy);
     };
...
void win::shift (int x1, int y1, int x2, int y2, int dx, int dy)
  {if (is_enable)
       /* perform_shift */      {XCopyArea (mydisplay,
                 mywindow,
                 mywindow,
                 mygc,
                 x1, y1, dx, dy, x2, y2);
...
void win::text_size (const char string [], int &dx, int &dy)
  {if (is_enable)
       /* perform_get_size */      {XGCValues gcinfo;
...
void win::write (int x, int y, const char string [])
  {if (is_enable)
       /* perform_write */      {XDrawImageString (mydisplay,
                        mywindow,
                        mygc,
                        x, y,
                        string,
...
int win::dx ()
  {if   (is_enable)
         /* calc_dx */      {XWindowAttributes attr;
...
int win::dy ()
  {if   (is_enable)
         /* calc_dy */      {XWindowAttributes attr;
...
void win::fix ()
  {table *fixes;
   ...
   if (with_fix)
       /* perform_fix */      { /* open_fixes */      {bool is_new;
  
      fixes = new table (fix_dir, fix_name, is_new);
      if (is_new)
          /* handle_dd */      {fixes->add_column ("name", col_type_text);
      ...
      fixes->add_column ("dy"  , col_type_int);
       ...
       /* get_entry */      {for (entry = 0; entry < fixes->number_of_rows; entry++)
        if (strcmp (fixes->read_text (entry, 0), name) == 0)
           break;
      if (entry == fixes->number_of_rows)
         entry = fixes->add_row ();
     };
       /* modify_entry */      {fixes->write (entry, 0, name);
...
void win::fix_pos (int &x, int &y)
  {table *fixes;
   int   e;
    ...
    /* get_entry */      {for (e = 0; e < fixes->number_of_rows; e++)
        if (strcmp (fixes->read_text (e, 0), name) == 0)
           break;
     };
    /* get_pos */      {if   (e != fixes->number_of_rows)
            /* read_data */      {x      = fixes->read_int (e, 1);
      y      = fixes->read_int (e, 2);
      open_x = x;
      open_y = y;
     }
      else  /* no_data */      {x = by_user;
      y = by_user;
     };
     };
   delete (fixes);
...
void win::fix_size (int &dx, int &dy)
  {table *fixes;
   int   e;
    ...
    /* get_pos */      {if   (e != fixes->number_of_rows)
            /* read_data */      {w_dx    = fixes->read_int (e, 3);
      w_dy    = fixes->read_int (e, 4);
      open_dx = w_dx;
      open_dy = w_dy;
     }
      else  /* no_data */      {dx = by_user;
      dy = by_user;
     };
     };
   delete (fixes);
...
void win::get_color (XImage *i, int x, int y, int &r, int &g, int &b)
  {XColor cc;
   ...
   cc.pixel = XGetPixel (i, x, y);
    /* get_rgb */      if   (is_last_pixel && cc.pixel == last_pixel)
           /* grab_last_rgb */      {r = last_r;
      g = last_g;
      b = last_b;
     }
     else  /* grab_new_rgb */      {XQueryColor (mydisplay, cmap, &cc);
      is_last_pixel = true;
      last_pixel    = cc.pixel;
      last_r        = r;
      last_g        = g;
...
void win::ppm (char file_name [], int x, int y, int dx, int dy)
  {FILE   *ppm_file;
   XImage *image;
    ...
    /* open_file */      {char name [128];

      sprintf (name, "%s.ppm", file_name);
      ppm_file = fopen (name, "w");
     };
    /* grab_image */      {get_image (image, x, y, dx, dy);
     };
    /* write_ppm */      { /* write_magic_number */      {fprintf (ppm_file, "P6\n");
     };
       /* write_name */      {fprintf (ppm_file, "#%s\n", file_name);
     };
       /* write_size */      {fprintf (ppm_file, "%d %d\n", dx, dy);
     };
       /* write_max_intensity */      {fprintf (ppm_file, "255\n");
     };
       /* write_picture */      {set_color (white);
      function  (GXxor);

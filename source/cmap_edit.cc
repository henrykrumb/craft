...
cmap_edit::cmap_edit(char map_name[])
{
    ...
    /* exec_show */ {
        for (int y = 0; y < dy; y++)
            for (int x = 0; x < dx; x++)
            /* show_pixel */ {
                map_win->set_color(/* act_map */ map[x][y]);
    ...
    /* show_color_table */ { /* open_color_table */
...
void cmap_edit::edit(bool &was_quit)
{
    ...
    /* perform_cmd */ {
                ...
                /* perform_load */ {
                    ...
                    /* exec_show */ {
                        for (int y = 0; y < dy; y++)
                            for (int x = 0; x < dx; x++)
                            /* show_pixel */ {
                                map_win->set_color(/* act_map_pixel */ map[x][y]);
                                map_win->fill(/* act_x_pos */ x_pic + x * (pixel_dx + 1), /* act_y_pos */ y_pic + y * (pixel_dy + 1), pixel_dx, pixel_dy);
        ...
        /* check_pixel_change */ {
            ...
            map_win->tick();
            if (map_win->is_mouse(xm, ym, button) && /* button_press */ (button == button1press || button == button2press || button == button3press) && /* within_pixels */ (x_pic <= xm && xm <= dx * (pixel_dx + 1) + x_pic && y_pic <= ym && ym <= dy * (pixel_dy + 1) + y_pic))
            /* handle_pixel_change */ {
                int new_color;
                int d;

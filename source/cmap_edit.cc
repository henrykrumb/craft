...
cmap_edit::cmap_edit (char map_name [])
   { /* store_params */      {strcpy (name, map_name);
     };
     /* init_status */      {changed    = false;
      fg_color   = 0;
     ...
     {if   (f_exists (complete (name, ".cmap")))
           load ();
      else  /* open_new_map */      { /* exec_resize */      {dx = 16;
      dy = 16;
     };
       /* fill_initial */      {for (int x = 0; x < max_map_dx; x ++)
        for (int y = 0; y < max_map_dy; y++)
           /* act_map */     map [x][y] = white;
     };
     };
      if (dx > 100 || dy > 100)
         {pixel_dx = 1;
     ...
     /* open_cmd_win */      {cmd_win = new win ("cmap_edit","",by_fix,by_fix,cmd_dx,cmd_dy);
      frame (cmd_win);
     };
     /* open_map_win */      {map_win = new win ("cmap_map", "", by_fix, by_fix, 
     ...
     /* init_omap */      {for (int x = 0; x < 5; x++)
        for (int y = 0; y < 5; y++)
          omap [x][y] = true;
      };
     /* exec_show */      {for (int y = 0; y < dy; y++)
        for (int x = 0; x < dx; x++)
           /* show_pixel */      {map_win->set_color ( /* act_map */     map [x][y]);
      map_win->fill      ( /* act_x_pos */   x_pic + x * (pixel_dx+1),  /* act_y_pos */   y_pic + y * (pixel_dy+1), pixel_dx, pixel_dy);
      picture->set_color ( /* act_map */     map [x][y]);
     ...
     /* show_color_table */      { /* open_color_table */      color_table = new win ("cmap_colors","",
                            by_fix,by_fix,colors_dx,colors_dy);
      for (int i = 0; i < max_color; i++)
         /* show_one_color */      {color_table->set_color (i);
      color_table->fill      ( /* x_color */  color_x + i * color_dx,  /* y_color */  color_y, color_dx-1, color_dy);
     };
     };
     /* show_colors */      { /* show_bg_color */      {cmd_win->set_color (bg_color);
      cmd_win->fill      (colors_x, colors_y, colors_d, colors_d);
     };
       /* show_colors_frame */      {cmd_win->set_color (black);
      ...
      cmd_win->line      (colors_x + colors_d, colors_y + colors_d, 
       ...
       /* show_fg_color */      {cmd_win->set_color (fg_color);
      cmd_win->fill      (colors_x + colors_d / 4, 
                           colors_y + colors_d / 4,
                           colors_d / 2, colors_d / 2);
     };
       /* show_t_color */      {cmd_win->set_color (t_color);
      cmd_win->fill      (colors_tx, colors_ty, colors_td, colors_td);
     };
      cmd_win->tick ();
     };
     /* open_buttons */      {bquit    = new button (cmd_win, "/quit",   bquit_x,   bquit_y);
...
cmap_edit::~cmap_edit ()
  { /* perhaps_save */      {if (changed && yes ("Save changes"))
        save ();
     };
   delete (bquit);
   delete (bsplit);
...
void cmap_edit::load ()
  { /* load_map */      {FILE *f;
       ...
       /* open_f */      {f_open (f, complete (name, ".cmap"), "r");
     };
       /* read_size */      {int d;

      fscanf (f, "%d %d %d %d", &d, &dx, &dy, &t_color);
     };
       /* read_data */      {for (int y = 0; y < dy; y++)
        for (int x = 0; x < dx; x++)
          fscanf (f, "%d", &map [x][y]);
     };
      fclose (f);
     };
    /* perhaps_load_mask */      {if (f_exists (complete (name, ".xbm")))
          /* load_mask */      {xbm *xb;
...
void cmap_edit::save_as (char new_name [], int x0, int y0, int sdx, int sdy)
  { /* store_name */      {strcpy (name, new_name);
     };
    /* save_map */      {FILE *f;

       /* open_f */      {f_open (f, complete (name, ".cmap"), "w");
     };
       /* write_size */      {fprintf (f, "0 %d %d %d\n", sdx, sdy, t_color);
     };
       /* write_data */      {for (int y = y0; y < y0 + sdy; y++)
         /* write_line */      {for (int x = x0; x < x0 + sdx; x++)
        fprintf (f, "%d ", map [x][y]);
      fprintf (f, "\n");
     };
     };
    ...
    /* clear_old_mask */      {char cmd [128];
 
      sprintf (cmd, "rm %s", complete (name, ".xbm"));
      system  (cmd);
     };
    /* perhaps_save_mask */      {bool is_mask;

       /* check_mask */      {is_mask = false;
      ...
      xb = new xbm (sdx, sdy);
       /* fill_xb */      {for (int x = x0; x < x0 + sdx; x++)
        for (int y = y0; y < y0 + sdy; y++)
          xb->set (x - x0, y - y0, (map [x][y] != t_color));
     };
       /* write_xb */      {xb->save (complete (name, ".xbm"));
     };
     };
     };
...
void cmap_edit::edit (bool &was_quit)
  { /* init_edit */      {was_quit = false;
     };
    /* perform_cmd */      {cmd_win->mark_mouse ();
      map_win->mark_mouse ();
       /* check_quit */      {if (bquit->eval ())
          /* handle_quit */      {bquit->press (true);
      was_quit = true;
     };
     };
       /* check_split */      {if (bsplit->eval ())
          /* handle_split */      {char new_name [128];
      int  sdx;
      int  sdy;
      int  no;

      bsplit->press (true);
       /* get_new_name */      {file_selector *f;
      ...
      delete (f);
       ...
       /* get_sdx_sdy */      {printf ("sdx sdy no :");
      scanf  ("%d %d %d", &sdx, &sdy, &no);
     };
      if ( /* file_should_be_saved */      (strlen (new_name) > 0 &&
     (! f_exists (complete (new_name, ".cmap")) || yes ("overwrite"))))
         split (sdx, sdy, new_name, no);
      bsplit->press (false);
     };
     };
       /* check_load */      {if (bload->eval ())
          /* handle_load */      {char new_name [128];
      ...
      if (cmap_sel (new_name, ".cmap"))
          /* perform_load */      {strcpy (name, new_name);
      load   ();
       /* exec_show */      {for (int y = 0; y < dy; y++)
        for (int x = 0; x < dx; x++)
           /* show_pixel */      {map_win->set_color ( /* act_map_pixel */ map  [x][y]);
      map_win->fill      ( /* act_x_pos */     x_pic + x * (pixel_dx+1),  /* act_y_pos */     y_pic + y * (pixel_dy+1), pixel_dx, pixel_dy);
      picture->set_color ( /* act_map_pixel */ map  [x][y]);
      picture->pixel     (pic_x + x, pic_y + y);
      ...
      bload->press (false);
       ...
       /* check_import */      {if (bimport->eval ())
          /* handle_import */      {char new_name [128];
      ...
      bimport->press (false);
       ...
       /* check_save */      {if (bsave->eval ())
          /* handle_save */      {bsave->press (true);
      save         ();
      changed = false;
      bsave->press (false);
     };
      if (bsaveas->eval ())
          /* handle_saveas */      {char new_name [128];
     ...
     (! f_exists (complete (new_name, ".cmap")) || yes ("overwrite"))))
      ...
      bsaveas->press (false);
       ...
       /* check_resize */      {if (bresize->eval ())
          /* handle_resize */      {bresize->press (true);
       /* perform_init */      {printf ("Enter size (dx, dy) :\n");
      scanf  ("%d %d", &dx, &dy);
      if (dx > 100 || dy > 100)
         {pixel_dx = 1;
          pixel_dy = 1;
      ...
      bresize->press (false);
       ...
       /* check_flip */      {if (bflip->eval ())
          /* handle_flip */      {bflip->press (true);
      flip_lr ();
       /* exec_show */      {for (int y = 0; y < dy; y++)
        for (int x = 0; x < dx; x++)
           /* show_pixel */      {map_win->set_color ( /* act_map_pixel */ map  [x][y]);
      ...
      bflip->press (false);
       ...
       /* check_replace */      {if (breplace->eval ())
          /* handle_replace */      {breplace->press (true);
      for (int x = 0; x < dx; x++)
        for (int y = 0; y < dy; y++)
          if (map [x][y] == fg_color)
             map [x][y] = bg_color;
      ...
      breplace->press (false);
       ...
       /* check_overlay */      {if (boverlay->eval ())
          /* handle_overlay */      {char new_name [128];
      ...
      boverlay->press (false);
       ...
       /* check_paste */      {if (bpaste->eval ())
          /* handle_paste */      {char new_name [128];
      ...
      bpaste->press (false);
       ...
       /* check_rotate */      {if (brotate->eval ())
          /* handle_rotate */      {brotate->press (true);
       /* perform_rotate */      {int orig [800][800];
      int rot  [800][800];

      changed = true;
       /* grab_orig */      {for (int x = 0; x < dx; x++)
        for (int y = 0; y < dy; y++)
          orig [x][y] =  /* act_map_pixel */ map  [x][y];
     };
       /* calc_rot */      {for (int i = 0; i < i_min (dx/2, dy/2); i++)
        rotate (orig, rot, i, i, dx - i - 1, dy - i - 1);
     };
       /* load_map */      {for (int x = 0; x < dx; x++)
        for (int y = 0; y < dy; y++)
           /* act_map_pixel */ map  [x][y] = rot [x][y];
     };
     };
       /* exec_show */      {for (int y = 0; y < dy; y++)
        for (int x = 0; x < dx; x++)
      ...
      if (brotateh->eval ())
          /* handle_rotateh */      {brotateh->press (true);
      rotate_rh ();
       /* exec_show */      {for (int y = 0; y < dy; y++)
        for (int x = 0; x < dx; x++)
           /* show_pixel */      {map_win->set_color ( /* act_map_pixel */ map  [x][y]);
      ...
      if (color_table->mouse (d, d, xm, ym, button))
          /* handle_color_change */      {int new_color;

       /* get_new_color */      {new_color = (xm-color_x) / color_dx;
     };
      if (button == button1press) fg_color = new_color;
      if (button == button2press)  /* handle_t_change */      {for (int x = 0; x < dx; x++)
        for (int y = 0; y < dy; y++)
          if ( /* act_map */       map  [xm][ym] == t_color)
              /* new_t_pixel */      {int x = xm;
      int y = ym;

       /* act_map */       map  [xm][ym] = new_color;
       /* show_pixel */      {map_win->set_color ( /* act_map_pixel */ map  [x][y]);
      map_win->fill      ( /* act_x_pos */     x_pic + x * (pixel_dx+1),  /* act_y_pos */     y_pic + y * (pixel_dy+1), pixel_dx, pixel_dy);
      picture->set_color ( /* act_map_pixel */ map  [x][y]);
      picture->pixel     (pic_x + x, pic_y + y);
      ...
      cmd_win->tick ();
       ...
       /* check_pixel_change */      {int xm;
      int ym;
      int button;

      map_win->tick ();
      if   (map_win->is_mouse (xm,ym,button) &&  /* button_press */      (button == button1press ||
      button == button2press ||
      button == button3press) &&  /* within_pixels */      (x_pic <= xm && xm <= dx * (pixel_dx+1) + x_pic &&
      y_pic <= ym && ym <= dy * (pixel_dy+1) + y_pic)) 
            /* handle_pixel_change */      {int new_color;
      int d;

       /* get_c_color */      {if (button == button1press)
           new_color = fg_color;
      else new_color = bg_color;
     };
       /* get_x_y */      {map_win->mouse (d, d, xm, ym, button);
      ...
      int yp = ym;
       ...
       /* calc_bounds */      {x0 = i_min (xm, last_x); xe = i_max (xm, last_x);
      ...
      else  /* show_pos */      {xm = (xm - x_pic) / (pixel_dx + 1);
      ym = (ym - y_pic) / (pixel_dy + 1);
      if (xcross != -1)
          /* clear_cross */      {for (int x = i_max (0, xcross - 1); x <= i_min (dx-1, xcross+1); x++)
      ...
      xcross = xm;
      ycross = ym;
       /* show_cross */      {picture->set_color (black);
      ...
      picture->tick ();
       ...
       /* check_left */      {if (bleft->eval ())
          /* handle_left */      {bleft->press (true);
       /* perform_left */      {int x;
      int y;

      for (x = 0; x < dx-1; x++)
        for (y = 0; y < dy; y++)
           /* act_map_pixel */ map  [x][y] = map [x+1][y];
      x = dx-1;
      for (y = 0; y < dy; y++)
         /* act_map_pixel */ map  [x][y] = bg_color;
     };
       /* exec_show */      {for (int y = 0; y < dy; y++)
        for (int x = 0; x < dx; x++)
           /* show_pixel */      {map_win->set_color ( /* act_map_pixel */ map  [x][y]);
      ...
      bleft->press (false);
       ...
       /* check_right */      {if (bright->eval ())
          /* handle_right */      {bright->press (true);
       /* perform_right */      {int x;
      int y;
      ...
      bright->press (false);
       ...
       /* check_up */      {if (bup->eval ())
          /* handle_up */      {bup->press (true);
       /* perform_up */      {int x;
      int y;

      for (x = 0; x < dx; x++)
        for (y = 0; y < dy-1; y++)
           /* act_map_pixel */ map  [x][y] = map [x][y+1];
      y = 0;
      for (x = 0; x < dx; x++)
         /* act_map_pixel */ map  [x][y] = bg_color;
     };
      ...
      bup->press (false);
       ...
       /* check_down */      {if (bdown->eval ())
          /* handle_down */      {bdown->press (true);
       /* perform_down */      {int x;
      int y;
      ...
      if (picture->mouse (xm, ym, b))
          /* load_omap */      {char name [128];

      xm = (xm - pic_x * 2) / dx;
      ym = (ym - pic_y * 2) / dy;
      if   (cmap_sel (name, ".cmap"))
            /* show_map */      {cmap *c = new cmap (picture, name);
      ...
      picture->scratch_mouse ();
       ...
       /* check_mark */      {if (d_mark->eval (d_is_mark))
         { /* clear_cross */      {for (int x = i_max (0, xcross - 1); x <= i_min (dx-1, xcross+1); x++)
        for (int y = i_max (0, ycross - 1); y <= i_min (dy-1, ycross+1); y++)
          if (is_mark)
             {picture->set_color (map [x][y]);
...
void cmap_edit::flip_ud ()
  {int b [dx][dy];
    ...
    /* exec_flip */      {for (int x = 0; x < dx; x++)
        for (int y = 0; y < dy; y++)   
          b [x][dy-y-1] = map [x][y];
     };
    /* store_result */      {for (int x = 0; x < dx; x++)
        for (int y = 0; y < dy; y++)
          map [x][y] = b [x][y];
     };
...
void cmap_edit::recolor (int n)
  {for (int x = 0; x < dx; x++)
     for (int y = 0; y < dy; y++)
      ...
      else  /* recolor_cyan */      {switch (map [x][y])
...
void cmap_edit::colorize (int ddx, int ddy, int c, int n)
  {int xx [500];
   ...
   if   (ddx == 0)
         /* handle_ddy */      {for (int i = 0; i < n; i++)
         /* pixel_ddy */      {int x;
      int y;

       /* rnd */      {bool in_list;

       /* get_xy */      {x = i_random (0, dx-1);
      y = i_random (0, dy-1);
     };
       /* check_list */      {in_list = true;
      for (int j = 0; j < num; j++)  
        if (xx [j] == x && yy [j] == y)
           break;
      in_list = false;
   ...
   else  /* handle_ddx */      {for (int i = 0; i < n; i++)
         /* pixel_ddx */      {int x;
      int y;
...
void cmap_edit::colorize (int ddx, int ddy)
  {if   (ddx == 0)
   ...
   else  /* handle_ddx */      {int xmin;
      int xmax;

       /* get_x_minmax */      {if   (ddx > 0)
           {xmin = 0;
            xmax = ddx;
           }
      else {xmin = dx + ddx;
...
void cmap_edit::rotate_rh ()
  {int orig [100][100];
   int rot  [100][100];
    ...
    /* calc_rot */      {int x0 = dx / 2;
      int y0 = dy / 2;

      for (int x = 0; x < dx; x++)
        for (int y = 0; y < dy; y++)
           /* rotate_point */      {point r = ::rotate (new_point (x, y, 0),
...
void cmap_edit::overlay (char name [])
  {int o_map [max_map_dx][max_map_dy];
   ...
   int dyy;
    ...
    /* read_o_map */      {int  version;
      int  bg;
      FILE *f;

       /* open_image_file */      {char i_file_name [256];
      ...
      fscanf (f, "%d %d %d %d", &version, &dxx, &dyy, &bg);
       ...
       /* read_pixels */      {for (int y = 0; y < dyy; y++)
        for (int x = 0; x < dxx; x++) 
           /* read_pixel */      {fscanf (f, "%d", &o_map [x][y]);
     };
     };
      fclose (f);
     };
    /* perform_overlay */      {for (int x = 0; x < dx; x++)
        for (int y = 0; y < dy; y++)
          if (map [x][y] == t_color)
              /* overlay_pixel */      {map [x][y] = o_map [x % dxx][y % dyy];
     };
     };
...
void cmap_edit::paste (char name [], int x0, int y0)
  {int o_map [max_map_dx][max_map_dy];
   ...
   int dyy;
    ...
    /* perform_overlay */      {for (int x = 0; x < dxx; x++)
        for (int y = 0; y < dyy; y++)
          if (map [x0+x][y0+y] == t_color)
              /* overlay_pixel */      {map [x0+x][y0+y] = o_map [x][y];
     };
     };
...
void cmap_edit::set_to_ppm (char map_name [], char ppm_name [])
  {ppm *p;
   win *w;
    ...
    /* open_ppm */      {p = new ppm (ppm_name);
     };
    /* open_map */      {w = new win ("lala", "", 10, 10, 10, 10);
